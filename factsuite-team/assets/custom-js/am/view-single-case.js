...
function load_case(candidate_id){
    ...
    $.ajax({
        ...
        if (data.length > 0) {
            ...
            $('#tatStartDays').html(data[0]['tat_start_date'])
            $('#tatEndDays').html(data[0]['tat_end_date'])
            $('#tatPauseDays').html(data[0]['tat_pause_date'])
            $('#tatReStartDays').html(data[0]['tat_re_start_date']);
    ...
    if(data.status > 0){
        ...
        if(type_of_degree.length > 0){
            ...
            // for (var i = 0; i < type_of_degree.length; i++) {
                ...
                html += '            <div class="pg-frm">';
                html += '               <label>Education type</label><br>';
                html += '               <label>Part Time</label>'; 
                html += '            </div>';
                html += '         </div>';
                ...
                if(data.component_data.gov_utility_bill != null || data.component_data.gov_utility_bill != ''){
                    ...
                    for (var i = 0; i < govUtilityBillDoc.length; i++) {
                        if ((/\.(jpg|jpeg|png)$/i).test(govUtilityBillDoc[i])){
                            ...
                            html += '                           <li class="image-selected-name pb-0">'+govUtilityBillDoc[i]+'</li>'
                            html += '                           <li class="image-name-delete pb-0">';
                            html += '                               <a download id="docs_modal_file'+data.component_data.candidate_id+'" href="'+img_base_url+"../uploads/gov-docs/"+passport_doc[i]+'" class="image-name-delete-a">';
                            html += '                                   <i class="fa fa-arrow-down">';
                            html += '                               </a>'; 
                            html += '                           </li>';
                            html += '                        </ul>';
    ...
    if(component_status.length > 0 ){
        ...
        for (var i = 0; i < component_status.length; i++) {  
            ...
            $landlord = landlord_name[i]['landlord_name']?landlord_name[i]['landlord_name']:'';
              ...
              $tlname = '';
            if (tenant_name) {
                $tlname = tenant_name[i]['tenant_name']?tenant_name[i]['tenant_name']:'';
            }
            // html += '<h6 class="full-nam2">Reference '+(j++)+'</h6>';
            html += '         <div class="row">';
            html += '            <div class="col-md-6">';
            ...
            html += '                  <input name="" readonly value="'+$tlname+'" class="fld form-control tenant_name" type="text">';
            html += '               </div>';
            html += '            </div>';
            html += '            <div class="col-md-4">';
            html += '               <div class="pg-frm">';
