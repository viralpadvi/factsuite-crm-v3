    ...
    if (files.length <= max_vendor_document_select) {
        ...
        if (files[0].size <= vendor_document_size) {
	        for (var i = 0; i < files.length; i++) {
	            ...
	            var html = '<div class="col-md-12 mt-3 pl-0 pr-0" id="file_vendor_documents_'+i+'">'+
...
function check_vendor_skill_tat(id) {
	...
	if (vendor_skill_tat != '') {
		...
		$('#vendor-tat-error-msg-div'+id).html('<span class="text-danger error-msg-small">Please enter the tat</span>');
		input_is_invalid('#vendor-tat'+id);
	}
}
...
function check_vendor_spoc_email_id() {
	...
	var vendor_id = $('#edit-vendor-id').val();
	if (vendor_spoc_email_id != '') {
		if(!email_regex.test(vendor_spoc_email_id)) {
        	...
        	input_is_invalid('#vendor-spoc-email-id');
      		...
      		$.ajax({
				type: "POST",
			  	url: base_url+"admin_Vendor/check_update_vendor_spoc_email_id_exists",
			  	data: { 
			  		vendor_id : vendor_id,
...
function cancel_add_new_vedor() {
	...
	$('#vendor-confirm-password-error-msg-div').html('&nbsp;');
...
function edit_vendor_details(vendor_id) {
	var vendor_name = $('#vendor-name').val();
	var vendor_address_line_1 = $('#vendor-address-line-1').val();
	...
	if (vendor_name != '' && alphabets_only.test(vendor_name) && vendor_name.length <= vendor_name_length 
		&& vendor_address_line_1 != '' && vendor_city != '' && alphabets_only.test(vendor_city) && vendor_city.length <= city_name_length && vendor_zip_code != '' && !isNaN(vendor_zip_code) && vendor_zip_code.length == vendor_zip_code_length && vendor_state != '' && vendor_website != '' && url_regex.test(vendor_website) && vendor_docs.length >= 0 && vendor_docs.length <= max_vendor_document_select
		&& vendor_monthly_quota != '' && !isNaN(vendor_monthly_quota) && vendor_monthly_quota.length <= vendor_monthly_quota_length 
		&& vendor_aggrement_start_date != '' && vendor_aggrement_end_date != '' && date_difference >= 0 
		&& vendor_skill_tat != ''  && vendor_skill_tat.length > 0 && vendor_manager_name != '' && vendor_spoc_name != '' 
		&& alphabets_only.test(vendor_spoc_name) && vendor_spoc_name.length <= vendor_spoc_name_length && vendor_spoc_email_id != '' 
		&& email_regex.test(vendor_spoc_email_id) && vendor_spoc_mobile_number != '' && !isNaN(vendor_spoc_mobile_number) 
		&& vendor_spoc_mobile_number.length == mobile_number_length && vendor_skills != '' && vendor_skills != null 
		&& vendor_skills != undefined && check_skill_empty_count == 0 && vendor_skills.length > 0) {
			$('#vendor-skill-error-msg-div').html('&nbsp;');
			...
			$.ajax({
				type: "POST",
			  	...
			  	success: function(data) {
				  	if (data.count == '0') {
				  		$('#edit-vendor-main-error-msg-div').html('<span class="text-warning error-msg-small">Please wait while we are updating.</span>');
				  		btn_disabled();
						...
						$.ajax({
							type: "POST",
						  	url: base_url+"admin_Vendor/edit_vendor_details",
						  	data: formdata,
						  	dataType: "json",
						  	contentType: false,
		    				processData: false,
						  	success: function(data) {
						  		$('#edit-vendor-main-error-msg-div').html('');
							  	if (data.status == '1') {
							  		$('#vendor-name, #vendor-address-line-1, #vendor-address-line-2, #vendor-city, #vendor-zip-code, #vendor-state, #vendor-website, #vendor-monthly-quota, #vendor-aggrement-start-date, #vendor-aggrement-end-date, #vendor-tat, #vendor-manager-name, #vendor-manager-email-id, #vendor-manager-mobile-number, #vendor-spoc-name, #vendor-spoc-email-id, #vendor-spoc-mobile-number').val('');
							  		$('#vendor-name, #vendor-address-line-1, #vendor-address-line-2, #vendor-city, #vendor-zip-code, #vendor-state, #vendor-website, #vendor-monthly-quota, #vendor-aggrement-start-date, #vendor-aggrement-end-date, #vendor-tat, #vendor-manager-name, #vendor-manager-email-id, #vendor-manager-mobile-number, #vendor-spoc-name, #vendor-spoc-email-id, #vendor-spoc-mobile-number').removeClass('is-valid is-invalid');
							  		$('.vendor-skills:checkbox:checked').prop("checked", false);
							  		$('#vendor-confirm-password-error-msg-div').html('&nbsp;');
							  		$('#selected-vendor-docs-li').html('');
							  		$('#edit_vendor_details_module').modal('hide');
							  		vendor_docs = [];
							  		toastr.success('Vendor details has been updated successfully.');
							  		if (active_type == 1) {
							  		...
							  		toastr.error('OOPS! Something went wrong while updating the vendor. Please try again.');
						  		}
						  	},
						  	error: function(data) {
						  		toastr.error('OOPS! Something went wrong while updating the vendor. Please try again.');
						  		btn_enabled(vendor_id);
						  	}
						});
				  	} else {
				  		$('#edit-vendor-main-error-msg-div').html('<span class="text-danger error-msg-small">Entered SOPC email id already exists. Please enter the new email id.</span>');
				  		btn_enabled(vendor_id);
				  		check_vendor_spoc_email_id();
			  		}
			  	}
		...
		$('#edit-vendor-main-error-msg-div').html('<span class="text-danger error-msg-small">Please fill all the necessary details.</span>');
...
function btn_disabled() {
	$('#edit-vendor-btn').removeAttr('onclick');
}

function btn_enabled(vendor_id) {
