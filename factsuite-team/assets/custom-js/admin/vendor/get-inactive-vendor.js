...
function get_inactive_vendor_list() {
	...
	$.ajax({
	    type  : 'ajax',
	    url   : base_url+'admin_Vendor/get_inactive_vendor_list',
	    dataType : 'json',
	    success : function(data) {
		          	...
		          	html += '<tr id="tr-'+data[i].vendor_id+'">'+
			            	...
			            	'<td>'+
			            		'<div class="custom-control custom-switch d-inline" id="change_status_check_div_'+data[i].vendor_id +'">'+
					            	'<input type="checkbox" onclick="change_vendor_status('+data[i].vendor_id +','+data[i].vendor_status+')" class="custom-control-input" id="change_vendor_status_'+data[i].vendor_id +'">'+
	    	...
	    	$('#get-inactive-vendor-list').html(html);   
	  	}
  	});
}
