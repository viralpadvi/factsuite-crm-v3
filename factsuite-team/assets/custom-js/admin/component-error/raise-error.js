...
function get_ticket_priority_list() {
    $.ajax({
        type: "POST",
        url: base_url+"factsuite-admin/get-ticket-priority-list", 
        dataType: "json",
...
function get_ticket_classification_list() {
    $.ajax({
        type: "POST",
        url: base_url+"factsuite-admin/get-ticket-classification-list", 
        dataType: "json",
        success: function(data) {
            var html = '<option value="">None</option>';
            for (var i = 0; i < data.length; i++) {
...
function add_new_error_modal(candidate_id,component_id,index) {
    $('#raise-error-btn').attr('onclick','raise_new_error('+candidate_id+','+component_id+','+index+')');
    $('#add-new-error-modal').modal('show');
}
...
function raise_new_error(candidate_id,component_id,component_index) {
    ...
    if (error_description != '') {
        ...
        $.ajax({
            type: "POST",
            url: base_url+"factsuite-admin/raise-new-error",
            data:formdata,
            dataType: 'json',
            contentType: false,
            processData: false,
