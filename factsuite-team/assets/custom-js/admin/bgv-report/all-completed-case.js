...
function loadAllAssignedCases(){
    $.ajax({
        type: "POST",
        url: base_url+"Admin_Main_Controller/completedCaseList", 
        dataType: "json",
        success: function(data) {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) { 
                    ...
                    html += '<td class="text-center d-none" id="genrate_'+data[i].candidate_id+'"><a id="genrate_report_'+data[i].candidate_id+'"  href="'+base_url+'outPutQc/htmlGenrateReport/'+data[i].candidate_id+'"  class="insuf-btn"><i class="fa fa-file" aria-hidden="true"></i></a></td>';
                    // html += '<td class="text-center" id="download_genrate_'+data[i].candidate_id+'"><a id="download_genrate_report_'+data[i].candidate_id+'"  href="'+base_url+'outPutQc/pdf/'+data[i].candidate_id+'"  class="insuf-btn"><i class="fa fa-download" aria-hidden="true"></i></a></td>';
                    html += '<td class="text-center" id="download_genrate_'+data[i].candidate_id+'"><a id="download_genrate_report_'+data[i].candidate_id+'"  href="javascript:void(0)" onclick="download_bgv_report('+data[i].candidate_id+')" class="insuf-btn"><i class="fa fa-download" aria-hidden="true"></i></a></td>';
                    html += '</tr>';
                    j++; 
                }
        } 
...
function download_bgv_report(candidate_id) {
    $.ajax({
        type: "POST",
        url: base_url+"factsuite-admin/check-generated-report-log", 
        data: {
        ...
        dataType: "json",
        success: function(data) {
            if(data.generated_report.length > 0) {
                var html = '',
                    generated_report = data.generated_report;
                for (var i = 0; i < generated_report.length; i++) {
                    html += '<tr>';
                    html += '<td>Version '+generated_report[i].report_generated_version+'</td>';
                    html += '<td>'+generated_report[i].report_generated_by_role+'</td>';
                    html += '<td class="text-right"><a target="_blank" href="'+base_url+'factsuite-admin/download-bgv-reports/'+generated_report[i].candidate_generated_report_log_id+'" class="insuf-btn"><i class="fa fa-download" aria-hidden="true"></i></a></td>';
                    html += '</tr>';
                }
                html += '<tr>';
                html += '<td colspan="2">Genarate New Report</td>';
                html += '<td class="text-right"><a target="_blank" href="'+base_url+'outPutQc/pdf/'+candidate_id+'" class="insuf-btn"><i class="fa fa-download" aria-hidden="true"></i></a></td>';
                html += '</tr>';
                $('#get-generated-report-log-data').html(html);
                $('#download-new-report-div-modal').html('<a target="_blank" href="'+base_url+'outPutQc/pdf/'+candidate_id+'" class="insuf-btn"><i class="fa fa-download" aria-hidden="true"></i></a>');
                $('#generated-report-log-modal').modal('show');   
            } else {
                $('#get-generated-report-log-data').html('');
                window.open(base_url+'outPutQc/pdf/'+candidate_id);
            }
        } 
    });
}
