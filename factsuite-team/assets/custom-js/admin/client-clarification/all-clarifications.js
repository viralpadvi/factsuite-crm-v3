...
function view_all_client_clarifications(candidate_id,component_id,component_index) {
	...
	$.ajax({
        type : "POST",
        url : base_url+"factsuite-admin/get-all-client-clarifications",
        data : formdata,
        contentType : false,
        processData : false,
        dataType : "json",
        success : function(data) {
            ...
            if (data.all_clarifications.length > 0) {
                for (var i = 0; i < all_clarifications.length; i++) {
                    ...
                    html += '<td id="clarification-log-created-date-'+all_clarifications[i].user_filled_details_component_client_clarification_id+'">'+all_clarifications[i]['client_clarification_created_date'].split(' ')[0]+'</td>';
...
function view_clarification_details(user_filled_details_component_client_clarification_id) {
	...
	$.ajax({
        type : "POST",
        url : base_url+"factsuite-admin/get-single-client-clarification-details",
        data : formdata,
        contentType : false,
        processData : false,
        dataType : "json",
        success : function(data) {
            ...
            if (clarification_details.client_clarification_classifications != '' && clarification_details.client_clarification_classifications != null) {
...
function add_new_client_clarification_comment(user_filled_details_component_client_clarification_id) {
    ...
    if (note_message != '') {
        ...
        $.ajax({
            type : "POST",
            url : base_url+"factsuite-admin/add-new-client-clarification-comment",
            data : {
...
function show_new_client_clarification_comment(user_filled_details_component_client_clarification_id,comment_id) {
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-admin/get-client-clarification-single-comment",
        data : {
        	verify_admin_request : 1,
            user_filled_details_component_client_clarification_id : user_filled_details_component_client_clarification_id,
        ...
        success: function(data) {
            if(data.clarification_details.status == 1) {
                ...
                var html = '<div class="single-timeline'+class_for_first_comment+'">';
                    ...
                    html += '<i class="far fa-clock"></i> '+data.clarification_details.clarification_comment.client_clarification_comment_created_date;
                    html += '</span>';
                    html += '<span> Added By : '+data.clarification_details.clarification_comment.component_client_clarification_commented_by_role+' ('+data.clarification_details.comment_added_by.first_name+' '+data.clarification_details.comment_added_by.last_name+')</span>';
...
function show_all_client_clarification_comments(user_filled_details_component_client_clarification_id,request_from = '') {
    ...
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-admin/get-single-client-clarifications-all-comments",
        data : {
        	verify_admin_request : 1,
            user_filled_details_component_client_clarification_id : user_filled_details_component_client_clarification_id
        },
            ...
            if(clarification_details.length > 0) {
                for (var i = 0; i < clarification_details.length; i++) {
                    ...
                    var html = '<div class="single-timeline'+class_for_first_comment+'">';
                        html += '<div class="timeline-item">';
                        html += '<span class="time">';
                        html += '<i class="far fa-clock"></i> '+clarification_details[i].client_clarification_comment_created_date;
                        html += '</span>';
                        var full_name = '';
                        ...
                        html += '<span> Added By : '+clarification_details[i].component_client_clarification_commented_by_role+' ('+full_name+')</span>';
...
function check_client_clarification_error_status(user_filled_details_component_client_clarification_id) {
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-admin/get-single-error-details",
        data : {
            user_filled_details_component_client_clarification_id : user_filled_details_component_client_clarification_id
        },
        dataType: "json",
