...
function get_filter_number_list() {
    $.ajax({
        type: "POST",
        url: base_url+"factsuite-admin/get-custom-filter-number-list", 
        dataType: "json",
        data : {
	  	...
	  	success: function(all_data) {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<td id="candidate_check'+data[i].candidate.candidate_id+'"><span class="tbody-th-span"><input type="checkbox" class="checkbox candidate-list" id="ids-'+data[i].candidate.candidate_id+'" name="ids[]" value="'+data[i].candidate.candidate_id+'" onclick="check_bulk_cases_selected_or_not()"></span></td>';
        	        // html += '<td>'+count+'</td>';
                    ...
                    html += '<td class="text-center"><a href="'+base_url+'factsuite-admin/view-case-detail/'+data[i].candidate.candidate_id+'" ><i class="fa fa-eye"></i></a></div>';
