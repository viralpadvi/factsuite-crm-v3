...
function load_case(candidate_id){
	...
	$.ajax({
        ...
        if (data.length > 0) {
            var j = 1;
            var is_report_generated = "No";
            if (data[0]['is_report_generated'] =='1') {
                is_report_generated = "Yes";
            }
            ...
            $('#report_generated_date').html(data[0]['report_generated_date']);
            $('#is_report_generated').html(is_report_generated);
            ...
            if (data[0]['document_uploaded_by'].toLowerCase() == 'candidate' || data[0]['signature'] !='1') {
                ...
                if (data[0]['is_submitted'] != 0) {
                    if (data[0]['uploaded_loa'] != null && data[0]['uploaded_loa'] != 'null' && data[0]['uploaded_loa']['signature_img'] != 'undefined' && data[0]['uploaded_loa']['signature_img'] != undefined && data[0]['uploaded_loa']['signature_img'] != '' && data[0]['uploaded_loa']['signature_img'] != null) {
                        $('#candidate-loa').html('LOA &nbsp;&nbsp;&nbsp;<a target="_blank" download href="'+img_base_url+'../uploads/doc_signs/'+data[0]['uploaded_loa']['signature_img']+'"><i class="fa fa-download"></i></a>');
                    }
                }
            }
            ...
            if(data[0]['candidate_id'] != null && data[0]['candidate_id'] != ''){
                ...
                if(data[0]['tat_pause_resume_status'] == '1' && data[0]['is_submitted'] !='2'){
                    ...
                    if (data[0]['is_submitted'] == 2) {
                        tatValue += '<span class="text-danger">Stop<span>';
                         $('#btn_tat_name').html('Stop TAT')
                         // $('#btn_pause_re_start_tat').attr('onclick','tat_confirm_dialog('+data[0]['candidate_id']+',"pause")')
                
                    }else if(data[0]['is_submitted'] == 0){
                         tatValue += '<span class="text-success">Start<span>';
            ...
            for (var i = 0; i < data.length; i++) { 
                ...
                $('#assigned-to-input-qc-date').html(((data[0]['candidate_details']['created_date'] != '' && data[0]['candidate_details']['created_date'] != null) ? get_date_formate(data[0]['candidate_details']['created_date']) : "-"));
                var inputqc_status_date = '-',
                    inputqc_verification_days_taken = 0,
                ...
                if(data[i].inputqc_status_date != '' && data[i].inputqc_status_date != null && data[i].inputqc_status_date != 'NA') {
                    ...
                    case_submitted_date_time_splitted = data[0]['candidate_details']['case_submitted_date'].split(' ')[0];
                       ...
                       var inputqc_status_date_time_splitted = inputqc_status_date.split(' ')[0],
                   ...
                   if (case_submitted_date_time_splitted !='--' && case_submitted_date_time_splitted !=null && inputqc_status_date_time !='-'  && inputqc_status_date_time !='--' && inputqc_status_date_time !=null ) {

                        inputqc_verification_days_taken = daysdifference(case_submitted_date_time,inputqc_status_date_time);
                       }
                        // inputqc_status_date = get_date_formate(inputqc_status_date);

                    if(inputqc_verification_days_taken > 0 && inputqc_verification_days_taken !='NaN') {
                        inputqc_verification_days_taken += ' Days';
                    } else {
                        inputqc_verification_days_taken = '0 Day';
                    
                    }
                }
                $('#verified-by-input-qc-date').html(get_date_formate(inputqc_status_date));
                ...
                if(data[i].inputqc_status_date != '' && data[i].inputqc_status_date != null && data[i].inputqc_status_date != 'NA' &&
                    ...
                    var analyst_status_date_time_splitted = analyst_specialist_status_date.split(' ')[0],
                        ...
                        inputqc_status_date = get_date_formate(inputqc_status_date),
                        analyst_specialist_status_date = get_date_formate(analyst_specialist_status_date);
                    if(analyst_specialist_verification_days_taken > 0) {
                        analyst_specialist_verification_days_taken += ' Days';
                    } else {
                        analyst_specialist_verification_days_taken = '0 Day';
                    }
                }
        ...
        success: function(data){
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<tr id="tr_'+data[i].tat_log_id+'">'; 
                    html += '<td>'+(j++)+'</td>'; 
                    html += '<td>'+get_date_formate(data[i].tat_start_date)+'</td>'; 
                    html += '<td>'+get_date_formate(data[i].tat_pause_date)+'</td>'; 
                    html += '<td>'+get_date_formate(data[i].tat_re_start_date)+'</td>'; 
                    html += '<td>'+get_date_formate(data[i].tat_end_date)+'</td>'; 
     ...
     $.ajax({
        ...
        success: function(data){ 
            ...
            if (data.status == '1' && data.logStatus == '1') {
                ...
                toastr.error('assignment has been update successfully. but log data has not been inserted.');
            }else{
    ...
    $.ajax({
        ...
        success: function(data){ 
            ...
            if (data.status == '1' && data.logStatus == '1') {
                ...
                toastr.error('Priority has been update successfully. but log data has not been inserted.');
            }else{
...
function click_to_call_get_details_for_modal(candidate_id) {
    $('#manual-call-sms').html('');
    $('#click-to-call-btn').html('<div class="spinner-border text-light custom-spinner-size"></div>');
    $('#click-to-call-btn').removeAttr('onclick');
    $.ajax({
        url: "../controller/get-single-candidate-details.php",
        type: "POST",
        dataType : 'JSON',
        data: {
            candidate_id : candidate_id
        ...
        success: function(data) {
            $('#click-to-call-btn').html('Call');
            $('#click-to-call-show-candidate-name').html(data.candidate_first_name+' '+data.candidate_last_name);
            $('#click-to-call-show-candidate-phone-number').html(data.candidate_phone_Number);
            $('#click-to-call-btn').attr('onclick','click_to_call_candidate(\''+candidate_id+'\')');
        }
    });
}

function click_to_call_candidate(candidate_id) {
    $('#click-to-call-btn').html('<div class="spinner-border text-light custom-spinner-size"></div>');
    $('#click-to-call-btn').removeAttr('onclick');
    $.ajax({
        url: base_url+"team/calling_system",
        type: "POST",
        dataType : 'JSON',
        data: {
            candidate_id : candidate_id
        ...
        success: function(data) {
             $("#click-to-call-candidate-modal").modal('hide')
            if (data.success) {
                $('#manual-call-sms').html('<span class="text-success error-msg">Call successfully initiated to the candidate.</span>');
                // get_manual_call_count(atob(candidate_id));
            } else {
                $('#manual-call-sms').html('<span class="text-danger error-msg">Call missed by agent.</span>');
            }
            $('#click-to-call-btn').html('Call');
            $('#click-to-call-btn').attr('onclick','click_to_call_candidate(\''+candidate_id+'\')');
        }
...
function get_call_details(candidate_id,first_name,last_name,mobile_number){ 
    $('#manual-call-sms').html('');
    $("#click-to-call-candidate-modal").modal('show')
    $('#click-to-call-btn').html('Call');
            $('#click-to-call-show-candidate-name').html(first_name+' '+last_name);
            $('#click-to-call-show-candidate-phone-number').html(mobile_number);
            $('#click-to-call-btn').attr('onclick','click_to_call_candidate(\''+candidate_id+'\')');
}
