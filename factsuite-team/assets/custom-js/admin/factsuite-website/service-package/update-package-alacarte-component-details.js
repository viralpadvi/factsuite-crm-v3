		...
		success: function(data) {
			...
			if (data.status == '1') {  
				if(data.service_package_details.status == 1) {
					toastr.success('Service package alacarte component details has been updated successfully.'); 
					setTimeout(function() { 
						window.location.href = base_url+'factsuite-admin/all-website-packages';
					}, 2000);
				} else {
					toastr.error('Something went wrong while updating the product compenent alacarte details. Please try again.');
				}
            } else { 
                toastr.error('Something went wrong while updating the package alacarte details. Please try again.'); 
