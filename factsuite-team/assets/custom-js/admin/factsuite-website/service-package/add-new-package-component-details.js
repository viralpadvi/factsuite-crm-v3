...
$('#client-save-packages-component-btn').on('click',function() {
	...
	$('#package-component-error').html('<span class="text-warning error-msg-small">Please wait while we are adding the component details.</span>');
    $.ajax({
		type: "POST",
			...
			if (data.status == '1') {  
				if(data.service_package_details.status == 1) {
					if (selected_alacarte_component_list != '') {
                		window.location.href = base_url+'factsuite-admin/add-website-package-alacarte-component-details?package_type='+get_package_type+'&package_name='+get_package_name+'&package_id='+get_enc_package_id;
					} else {
						toastr.success('New service package component details has been added successfully.'); 
						setTimeout(function() { 
							window.location.href = base_url+'factsuite-admin/add-website-package';
						}, 2000);
					}
				} else {
					toastr.error('Something went wrong while adding the service package component details. Please try again.');
				}
