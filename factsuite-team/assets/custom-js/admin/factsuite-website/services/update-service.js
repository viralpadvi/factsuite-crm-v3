...
function check_service_name() {
	...
	variable_array['ajax_call_url'] = 'factsuite-admin/check-update-service-name';
	variable_array['ajax_pass_data'] = {verify_admin_request : 1, service_name : $('#service-name').val(), service_id : $('#modal-edit-service-id').val()};
...
function check_added_product_gallery_description(i) {
	var variable_array = {};
	variable_array['input_id'] = '#added-service-benefit-description-'+i;
	variable_array['error_msg_div_id'] = '#added-service-benefit-description-error-msg-div-'+i;
...
function update_service(service_id) {
	...
	var service_benefit_image_input_counter = 0;

	var added_service_benefit_image_input_i_array = [],
		service_benefits_image_input_array = [],
		added_service_benefit_image_input_array = [];

	if(service_benefits_images_array != '' && service_benefits_images_array.length > 0) {
		var service_benefits_image_input_i_array = [];
		$('.service-benefits-input-i').each(function(i) {
		...
		if (service_benefits_image_input_i_array.length > 0) {
			for (var i = 0; i < service_benefits_image_input_i_array.length; i++) {
	 			var check_service_benefit_description_var = check_service_benefits_description(service_benefits_image_input_i_array[i]);
		  		if (check_service_benefit_description_var == 0) {
		   			service_benefit_image_input_counter++;
		  		}
			}
		}
	}

	$('.added-service-benefit-image-input-i').each(function(i) {
		added_service_benefit_image_input_i_array[i] = $(this).val();
	});

	var service_benefit_image_sorting_ids = [];
  	$('.product-service-benefit-image-sorting-ui').each(function (index) { 
    	var id = $(this).attr('data-sorting_id');
    	service_benefit_image_sorting_ids.push(id); 
  	});

	if(added_service_benefit_image_input_i_array.length > 0) {
		for (var i = 0; i < added_service_benefit_image_input_i_array.length; i++) {
	 		var check_service_benefit_description_var = check_added_product_gallery_description(added_service_benefit_image_input_i_array[i]);
		  	if (check_service_benefit_description_var == 0) {
		   		service_benefit_image_input_counter++;
		  	}
		}
	...
	if (check_service_name_var == 1 && check_maximum_checks_var == 1 && check_checked_components_var == 1 && short_description != '' 
		 && very_short_description != '' && long_description != '' && service_included_description != '' && service_benefit_image_input_counter == 0) {
		$('#short-description-error-msg-div').html('');
		$('#long-description-error-msg-div').html('');
		$('#service-included-description-error-msg-div').html('');

		$('#edit-service-btn').removeAttr('onclick');
		$('#edit-service-btn').prop('disabled',true);
		$('#update-service-error-msg-div').html('<span class="text-warning error-msg">Please wait while we are updating service.</span>');
		...
		$('input[name="component-name[]"]:checked').each(function(i) {
   		...
   		var is_self_check = 0;
   		if ($('#is-self-check-available:checked').val() == 'on') {
   			is_self_check = 1;
   		}
		...
		if (service_benefits_images_array.length > 0) {
	   	...
	   	formdata.append('service_benefit_image_sorting_ids', service_benefit_image_sorting_ids);
		
		$('.added-service-benefit-image-input').each(function(i) {
	  		added_service_benefit_image_input_array.push($(this).val());
		});
		formdata.append('added_service_benefit_image_input_array', added_service_benefit_image_input_array);

		$.ajax({
			type: "POST",
		  	url: base_url+"factsuite-admin/update-factsuite-website-service",
		  	data:formdata,
		  	dataType: 'json',
		    contentType: false,
		    processData: false,
		  	success: function(data) {
		  		if (data.status == 1) {
			  		$('#edit-service-btn').prop('disabled',false);
					$('#update-service-error-msg-div').html('');
				  	if (data.service_details.status == '1') {
				  		$('#service-name-li-'+service_id).html(service_name);
						...
						$('#edit-service-modal').modal('hide');
				  		...
				  		toastr.error('Something went wrong while updating the service. Please try again.');
			  		}
			  	} else if (data.status == 2) {
			  		$('#edit-service-btn').attr('onclick','update_service('+service_id+')');
			  		$('#update-service-error-msg-div').html('<span class="text-danger error-msg-small">Entered service name already exists. Please enter a new service.</span>');
			  	} else {
			  		check_admin_login();
			  	}
		  	},
		  	error: function(data) {
		  		...
		  		$('#edit-service-btn').prop('disabled',false);
				$('#update-service-error-msg-div').html('');
		  		toastr.error('Something went wrong while updating the service logo. Please try again.');
		  	}
		});
	} else {
		$('#update-service-error-msg-div').html('<span class="text-danger error-msg">Please fill all the required details correctly.</span>');
