...
function view_ticket_details(approval_id) {
        ...
        success: function(data) {
                var action = 'Creating';
                    ...
                    $("#approval_id").val(data.approval_id)
                            var access = [];
                            var level_one = '<option value="">Assign to</option>';
                            var level_two = '<option value="">Assign to</option>';
                            var level_tree = '<option value="">Assign to</option>';
                            var status_level_one = '', status_level_two = '', status_level_tree = '';
                        ...
                        for (var i = 0; i < team.length; i++) {
                            ...
                            for (var k = 0; k < access.length; k++) {
                                ...
                                if (team[i].team_id == data.level_one_id) {
                                    select = 'selected';
                                    status_level_one += '( '+team[i].first_name+' )'; 
                                }
                               level_one += '<option '+select+' value="'+team[i].team_id+'">'+team[i].first_name+'</option>'; 
                            }
                            if (2== access[k]) {
                                var selecte ='';
                                if (team[i].team_id == data.level_two_id) {
                                    selecte = 'selected';
                                    status_level_two += '( '+team[i].first_name+' )'; 
                                }
                                level_two += '<option '+selecte+' value="'+team[i].team_id+'">'+team[i].first_name+'</option>';
                            }
                            if (3== access[k]) {
                                var selected ='';
                                if (team[i].team_id == data.approved_by) {
                                    selected = 'selected';
                                    status_level_tree += '( '+team[i].first_name+' )'; 
                                }
                                level_tree += '<option '+selected+' value="'+team[i].team_id+'">'+team[i].first_name+'</option>';
                            }
                            ...
                            $("#level_one").html(level_one);
                            $("#level_two").html(level_two);
                            $("#level_three").html(level_tree);
                   ...
                   var status_one = 'Pending';
                    ...
                    $("#status_level_one").html(status_one+status_level_one);
                    $("#status_level_two").html(status_two+status_level_two);
                    $("#status_level_three").html(status_three+status_level_tree);
    ...
    $("#level_one").on('change',function(){
       ...
       if (assign !='') { 
         // assign_to(1,assign)
         $("#accept-new-btn").attr('onclick','assign_to(1,'+assign+')');
       }
    })
    $("#level_two").on('change',function(){
    var assign = $(this).val();
     $("#accept-new-model").modal('show'); 
        if (assign !='') { 
         // assign_to(2,assign)
             $("#accept-new-btn").attr('onclick','assign_to(2,'+assign+')');
       }
    })
    $("#level_three").on('change',function(){
    var assign = $(this).val();
     $("#accept-new-model").modal('show');
        if (assign !='') { 
         // assign_to(3,assign)
             $("#accept-new-btn").attr('onclick','assign_to(3,'+assign+')');
       }
    })


function assign_to(level,assign){


    $.ajax({
        type : "POST",
        url : base_url+"Approval_Mechanisms/update_approval_assign",
        data : { 
        ...
        success: function(data) {
             ...
             toastr.success('Approval Assign Change has been updated successfully.');
...
function approve_the_mechanism(approval_id,status){
        $.ajax({
        type : "POST",
        url : base_url+"Approval_Mechanisms/update_approval_data",
        data : {
