          ...
          success: function(data) { 
          	...
          	var html='';
          		...
          		// for (var i = 0; i < data.pack.length; i++) { 
					...
					html +='</div>';
					if (data.pack['component_id'] == 3 || data.pack['component_id'] == 27) {
					html +=doc;
					}else if(data.pack['component_id'] == 4){
					html +=drug;
					}else if(data.pack['component_id'] == 7){
					html +=edu;
					}else if (!jQuery.inArray(data.pack['component_id'], [3,4,7,27]) !== -1) {
              ...
              success: function(data) {
                if (data.status == '1') {  
                	toastr.success('New product order has been updated successfully.'); 
                	 window.location.href = base_url+'factsuite-admin/edit-select-package-component-client/'+client_id;
...
$("#client-save-package-component-btn").on('click',function(){
	...
	window.location.href = base_url+'factsuite-admin/edit-client-component-packages/'+client_id;
});
...
var package_name = '';
	...
	for (var i = 0; i < package_id.length; i++) {
          	...
          	html +='<option data-name="'+package_name[i]+'" data-package_id="'+package_id[i]+'" value="'+package_id[i]+'">'+package_name[i]+'</option>';
...
var package_name = $("#component-package option:selected").data('name');
          ...
          success: function(data) {
          	...
          	// if (data.pack.length > 0) {
          		...
          		// for (var i = 0; i < data.pack.length; i++) { 
					...
					html +='<label class="custom-control-label" for="customCheck'+package_id+'_'+data.pack['component_id']+'">'+data.pack[component_config_name]+' ('+package_name+')</label>';
					html +='</div>';
					html +='</div>';
					html +='<div class="col-md-4 mt-1">';
					html +='<label>Component Standard Price</label>';
...
// alert(JSON.stringify(package_components))
	...
	var components_lenght = $("#valid_comp").val().split(',');
            ...
            type: "POST",
              ...
              success: function(data) {
                if (data.status == '1') {  
                	toastr.success('New product order has been updated successfully.'); 
                	 // window.location.href = base_url+'factsuite-admin/edit-select-package-component-client/'+client_id;
                	 window.location.href = base_url+'factsuite-admin/edit-client-alacarte-component/'+client_id;
            ...
            type: "POST",
              ...
              success: function(data) {
                if (data.status == '1') {  
                	...
                	// window.location.href = base_url+'factsuite-admin/view-all-client';
