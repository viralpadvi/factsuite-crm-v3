	  	...
	  	success: function(data){ 
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) { 
                    ...
                    $client_city = '-'; 
                    $industry_name = '-'; 
                    if(data[i]['spoc_email_id'] !=null){
                        $spoc_email_id = data[i]['spoc_email_id'];
                        $spoc_name = data[i]['spoc_name'] !=null ? data[i]['spoc_name']:'-';
                        $client_city = data[i]['client_city'] !=null ? data[i]['client_city']:'-';
                        $industry_name = data[i]['industry_name'] !=null ? data[i]['industry_name']:'-';
                    }
                	html += '<tr id="tr_'+data[i].client_id+'">'; 
                	html += '<td>'+j+'</td>';
                	html += '<td id="client_name_'+data[i].client_id+'">'+data[i]['client_name']+'</td>';
                	html += '<td id="client_industry_'+data[i].client_id+'">'+$industry_name+'</td>';
                	html += '<td id="client_city_'+data[i].client_id+'">'+$client_city+'</td>';
                	// html += '<td id="role_'+data[i].client_id+'">'+$spoc_name+'</td>';
                	// html += '<td id="reporting_manager_'+data[i].client_id+'">'+$spoc_email_id+'</td>';
                    if (access == 2) {
                        html += '<td class="text-center"><a  href="'+base_url+'factsuite-admin/edit-client/'+data[i].client_id+'"><i class="fa fa-pencil"></i></a></div>&nbsp;&nbsp;&nbsp;<a onclick="remove_client_field('+data[i].client_id+')" href="#"  class=" ml-1 " ><i class="fa  fa-trash text-danger"></i></a></td>';
                    }else{
                        html += '<td></td>';  
                    }
                    html += '<td class="text-center"><a onclick="view_client_spoc_list('+data[i].client_id+')" href="javascript:void(0)" class="ml-1"><i class="fa fa-eye"></i></a></td>';
...
function view_client_spoc_list(client_id) {
    if (client_id != '') {
        $.ajax({
            type: "POST",
            url: base_url+"factsuite-admin/get-client-spoc-list",
            data: {
            ...
            dataType: "json",
            success: function(data) { 
                let html = '<tr><td colspan="4" class="text-center">No Client SPOC Found.</td></tr>';
                if (data.client_spoc_list.length > 0) {
                    html = '';
                    var client_spoc_list = data.client_spoc_list;
                    for (var i = 0; i < client_spoc_list.length; i++) { 
                        html += '<tr>'; 
                        html += '<td>'+(i+1)+'</td>';
                        html += '<td>'+client_spoc_list[i]['spoc_name']+'</td>';
                        html += '<td>'+client_spoc_list[i]['spoc_email_id']+'</td>';
                        html += '<td class="text-center" id="send-credentials-to-client-spoc-'+client_spoc_list[i]['spoc_id']+'"><a onclick="send_credentials_to_spoc('+client_spoc_list[i]['spoc_id']+')" href="javascript:void(0)" class="text-dark">Send</a></td>';
                        html += '</tr>';
                    }
                }

                $('#get-client-spoc-list-data').html(html); 
                $('#show-client-spoc-list-modal').modal('show');
            } 
        });
    }
}
