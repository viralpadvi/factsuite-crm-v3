...
function get_all_assigned_tickets() {
	$.ajax({
        type : "POST",
        url : base_url+"factsuite-tech-support/get-all-assigned-tickets",
        data : {
        ...
        success: function(data) {
            ...
            if (data.all_tickets.length > 0) {
                for (var i = 0; i < all_tickets.length; i++) {
                    ...
                    html += '<td id="first_name'+all_tickets[i].ticket_id+'">'+all_tickets[i].ticket_created_by_role_id+'</td>';
                    ...
                    html += '<td id="start_date'+all_tickets[i].ticket_id+'">'+all_tickets[i]['ticket_created_date']+'</td>';
...
function view_ticket_details(ticket_id) {
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-tech-support/get-ticket-details",
        data : {
            ticket_id : ticket_id,
            verify_tech_support_request : 1
        },
...
function update_ticket_status(ticket_id) {
    ...
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-tech-support/update-ticket-status",
        data : {
            ticket_id : ticket_id,
...
function add_new_ticket_comment(ticket_id) {
    ...
    if (note_message != '') {
        ...
        $.ajax({
            type : "POST",
            url : base_url+"factsuite-tech-support/add-new-ticket-comment",
            data : {
                ticket_id : ticket_id,
...
function show_new_comment(ticket_id,comment_id) {
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-tech-support/get-ticket-single-comment",
        data : {
            ticket_id : ticket_id,
...
function show_all_comments(ticket_id) {
    ...
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-tech-support/get-single-ticket-all-comments",
        data : {
            ticket_id : ticket_id,
            verify_tech_support_request : 1
        },
