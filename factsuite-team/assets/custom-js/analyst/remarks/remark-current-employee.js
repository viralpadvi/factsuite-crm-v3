...
function current_emp(){
	...
	var in_progress_remarks = $("#in_progress_remarks").val();
	
	// alert(verification_remarks)
	// alert(in_progress_remarks)
	...
	formdata.append('index',$("#componentIndex").val()); 
	formdata.append('in_progress_remarks',in_progress_remarks);
  ...
  formdata.append('component_name','Current Employment');
	...
	$.ajax({
        type: "POST",
          url: base_url+"analyst/update_remarks_current_employment",
          data:formdata,
          dataType: 'json',
          contentType: false,
          processData: false,
...
$("#employee_id").on('keyup blur',employee_id); 
$("#department").on('keyup blur',department); 
$("#remark_salary_type").on('keyup blur change',remark_salary_type); 
$("#managers_designation").on('keyup blur',managers_designation);  
$("#remarks_reason_for_leaving").on('keyup blur',remarks_reason_for_leaving); 

$("#verification_remarks").on('keyup blur',verification_remarks); 
$("#in_progress_remarks").on('keyup blur',in_progress_remarks); 
$("#insuff_remarks").on('keyup blur',insuff_remarks); 
$("#insuff_closure_remarks").on('keyup blur',insuff_closure_remarks); 
$("#verification_fee").on('keyup blur',verification_fee); 
...
function employee_id(){
	var designation = $("#employee_id").val();
	if (designation !='') {
...
function department(){
	var designation = $("#department").val();
	if (designation !='') {
...
function remark_salary_type(){
	var designation = $("#remark_salary_type").val();
	if (designation !='') {
		...
		$("#remark_salary_type-error").html("<span class='text-danger error-msg-small'>Please Enter Valid salary type</span>");
		return 0
	}
}

function managers_designation(){
	var designation = $("#managers_designation").val();
	if (designation !='') {
		$("#managers_designation-error").html("&nbsp;");
		input_is_valid("#managers_designation")
		return 1
	}else{
		input_is_invalid("#managers_designation")
		$("#managers_designation-error").html("<span class='text-danger error-msg-small'>Please Enter Valid manager designation</span>");
		return 0
	}
}

function remarks_reason_for_leaving(){
	var designation = $("#remarks_reason_for_leaving").val();
	if (designation !='') {
		$("#remarks_reason_for_leaving-error").html("&nbsp;");
		input_is_valid("#remarks_reason_for_leaving")
		return 1
	}else{
		input_is_invalid("#remarks_reason_for_leaving")
		$("#remarks_reason_for_leaving-error").html("<span class='text-danger error-msg-small'>Please Enter Valid Inputs</span>");
		return 0
	}
}

function verification_remarks(){
	var designation = $("#verification_remarks").val();
	if (designation !='') {
		...
		$("#verification_remarks-error").html("<span class='text-danger error-msg-small'>Please Enter Valid remarks</span>");
		return 0
	}
}

function in_progress_remarks(){
	var designation = $("#in_progress_remarks").val();
	if (designation !='') {
		$("#in_progress_remarks-error").html("&nbsp;");
		input_is_valid("#in_progress_remarks")
		return 1
	}else{
		input_is_invalid("#in_progress_remarks")
		$("#in_progress_remarks-error").html("<span class='text-danger error-msg-small'>Please Enter Valid inprogress remarks</span>");
		return 0
	}
}

function insuff_remarks(){
	var designation = $("#insuff_remarks").val();
	if (designation !='') {
		$("#insuff_remarks-error").html("&nbsp;");
		input_is_valid("#insuff_remarks")
		return 1
	}else{
		input_is_invalid("#insuff_remarks")
		$("#insuff_remarks-error").html("<span class='text-danger error-msg-small'>Please Enter Valid insuff remarks</span>");
		return 0
	}
}

function insuff_closure_remarks(){
	var designation = $("#insuff_closure_remarks").val();
	if (designation !='') {
		$("#insuff_closure_remarks-error").html("&nbsp;");
		input_is_valid("#insuff_closure_remarks")
		return 1
	}else{
		input_is_invalid("#insuff_closure_remarks")
		$("#insuff_closure_remarks-error").html("<span class='text-danger error-msg-small'>Please Enter Valid closure remarks</span>");
		return 0
	}
}

function verification_fee(){
	var designation = $("#verification_fee").val();
	if (designation !='') {
		$("#verification_fee-error").html("&nbsp;");
		input_is_valid("#verification_fee")
		return 1
	}else{
		input_is_invalid("#verification_fee")
		$("#verification_fee-error").html("<span class='text-danger error-msg-small'>Please Enter Valid verification fee</span>");
		return 0
	}
}
