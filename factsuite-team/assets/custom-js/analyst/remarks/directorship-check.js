...
function directorship_check(){
  ...
  $(".verified_date").each(function(){
	...
	var directorship_check_id = $("#directorship_check_id").val();
	// analyst_status
	...
	formdata.append('directorship_check_id',directorship_check_id);
  ...
  formdata.append('component_name','Directorship Check');
	...
	$("#wait-message").html("<span class='text-warning'>Please wait while we are updating the data.</span>");
		$.ajax({
            type: "POST",
              url: base_url+"analyst/update_remarks_directorship_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
