...
function bankruptcy(){
  ...
  // alert(candidateInfo['insuff_remarks'])
  if(candidateInfo['insuff_remarks'] != 'null' && candidateInfo['insuff_remarks'] != null && candidateInfo['insuff_remarks'] != ""){
  ...
  formdata.append('component_name','Bankruptcy Check');
  ...
  $("#wait-message").html("<span class='text-warning'>Please wait while we are updating the data.</span>");
    $.ajax({
            type: "POST",
              url: base_url+"analyst/update_remarks_bankruptcy_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
