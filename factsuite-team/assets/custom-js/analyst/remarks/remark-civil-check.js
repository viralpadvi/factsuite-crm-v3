...
function handleFileSelect_candidate_aadhar(e){ 
    ...
    if (files.length <= max_client_document_select) {
    	$("#selected-civil-docs-li").html('');
        $("#civil-upoad-docs-error-msg-div").html('');
        if (files[0].size <= client_document_size) {
	        ...
	        for (var i = 0; i < files.length; i++) {
	            var fileName = files[i].name; // get file name 
	                ...
	                $("#civil-upoad-docs-error-msg-div").append(html);
	        	} 
	        }
	    } else {
	    	$("#selected-civil-docs-li").html('<div class="col-md-12"><span class="text-danger error-msg-small">Document size should be of max 20 Mb.</span></div>');
			$('#client-documents').val('');
	    }
    } else {
        $("#selected-civil-docs-li").html('<span class="text-danger error-msg-small">Please select a max of '+max_client_document_select+' files</span>');
    }
}
...
function civil_check(){
  ...
  formdata.append('component_name','Civil Status');
	...
	// if (verification_remarks.length == addressCount) {
		...
		$.ajax({
            type: "POST",
              url: base_url+"analyst/update_remarks_candidate_civil_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
