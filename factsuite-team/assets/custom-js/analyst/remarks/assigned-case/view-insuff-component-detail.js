...
function loadAllInsuffComponent(team_id){
    $.ajax({
        type: "POST",
        url: base_url+"analyst/getInsuffComponentForms",
        data:{
            team_id:team_id
        },
        dataType: "json",
            ...
            if(data.length > 0){ 
                ...
                for (var i = 0; i < data.length; i++) {
                   ...
                   if (data[i].analyst_status == '0' || data[i].analyst_status == '1') {
                        ...
                        analystQcStatus = '<span class="text-success">No Insuff<span>';
                    }else{
                    ...
                    html +='<td class="text-capitalize" >'+data[i].component_name+'</td>';
                ...
                $('#get-insuff-component-data').html(html)
