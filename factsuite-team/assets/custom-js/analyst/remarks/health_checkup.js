...
function health_check(){
  ...
  $(".verified_date").each(function(){
	...
	var health_checkup_id = $("#health_checkup_id").val();
	// analyst_status
	...
	formdata.append('health_checkup_id',health_checkup_id);
  ...
  formdata.append('component_name','Health Check');
	...
	$("#wait-message").html("<span class='text-warning'>Please wait while we are updating the data.</span>");
		$.ajax({
            type: "POST",
              url: base_url+"analyst/update_remarks_health_checkup",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
