...
function permanent_address(){
	...
	// 		valid_verification_remarks()==1 && valid_property_type() == 1){
		...
		formdata.append('component_name','Permanent Address');
		...
		if ($('input[name="client-iverify-or-pv-type"]:checked').length > 0) {
			$.ajax({
		        type: "POST",
		          url: base_url+"analyst/update_remarks_candidate_permanent_address",
		          data:formdata,
		          dataType: 'json',
		          contentType: false,
		          processData: false,
...
$("#view_vendor_log").on('click',function(){
	...
	var component_id = $("#permanent_address_id").val();
