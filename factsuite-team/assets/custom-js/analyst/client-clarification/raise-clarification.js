...
$('#action_status').on('change', function() {
    if($('#action_status').val() == 8) {
        $('#add-new-client-clarification-modal').modal('show');
    }
});
...
function check_client_clarification_description() {
    var variable_array = {};
    variable_array['input_id'] = '#client_clarification_description';
    variable_array['error_msg_div_id'] = '#client-clarification-description-error-msg-div';
    variable_array['empty_input_error_msg'] = 'Please enter the clarification description';
    return mandatory_any_input_with_no_limitation(variable_array);
}
...
function raise_new_client_clarification() {
    var client_clarification_subject = $('#client-clarification-subject').val(),
        ...
        client_clarification_description = $('#client_clarification_description').val(),
    ...
    var check_client_clarification_subject_var = check_client_clarification_subject(),
        check_client_clarification_description_var = check_client_clarification_description();

    if (check_client_clarification_subject_var == 1 && check_client_clarification_description_var == 1) {
        ...
        formdata.append('verify_analyst_request',1);
        ...
        $.ajax({
            type: "POST",
            url: base_url+"factsuite-analyst/raise-new-client-clarification",
            data:formdata,
            dataType: 'json',
            contentType: false,
            processData: false,
