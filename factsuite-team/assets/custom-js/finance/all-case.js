...
function get_all_cases(request_from = '') {
    ...
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-finance/get-all-cases",
        data : {
        ...
        success: function(all_data) {
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    var all_client_packages = JSON.parse(data[i].all_client_packages),
                        total_amount = 0;
                    ...
                    html += '<td id="case_id'+data[i].candidate_id+'">'+data[i].candidate_id+'</td>';
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.new_cases_count > 0) {
                $('#load-more-btn-div').html('<button class="send-btn mt-0 mr-0 btn-filter-search mt-2" id="load-more-btn" onclick="get_all_cases(\'load_more\')">Load More</button>');
            } else {
