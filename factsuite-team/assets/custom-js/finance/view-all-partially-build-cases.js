...
$('#filter-cases-client-list').on('change', function() {
    get_all_partially_built_cases('client_search');
});

$('#filter-input').on('keyup', function() {
    var filter_input = $('#filter-input').val();
    if (filter_input == '') {
        get_all_partially_built_cases('filter_input');
    }
});
...
$('#load-more-btn').on('click', function() {
    get_all_partially_built_cases('load_more');
});
...
function get_all_partially_built_cases(request_from = '') {
    ...
    $.ajax({
        type : "POST",
        url : base_url+"factsuite-finance/get-all-partially-builled-cases",
        data : {
        ...
        success: function(all_data) {
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<td id="start_date'+data[i].candidate_id+'">'+moment(data[i]['created_date']).format(selected_datetime_format['js_code'])+'</td>';
                    html += '<td id="total_amount_'+data[i].candidate_id+'">'+data[i]['email_id']+'</td>';
                    html += '<td class"'+classStatus+'" id="status'+data[i].candidate_id+'">'+status+'</td>';
                    html += '<td class="text-center" id="genrate_'+data[i].candidate_id+'"><a id="genrate_report_'+data[i].candidate_id+'"  href="'+base_url+'factsuite-finance/view-case-details/'+data[i].candidate_id+'?request_from=all-cases"><i class="fa fa-eye" aria-hidden="true"></i></a></td>';
                    html += '</tr>';
            ...
            // setTimeout(function() {
                ...
                $('#all-case-filter-btn').attr('onclick','get_all_partially_built_cases(\'filter_input\')');
            // }, 1500);
        } 
    });
}
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.new_cases_count > 0) {
                $('#load-more-btn-div').html('<button class="send-btn mt-0 mr-0 btn-filter-search mt-2" id="load-more-btn" onclick="get_all_partially_built_cases(\'load_more\')">Load More</button>');
            } else {
