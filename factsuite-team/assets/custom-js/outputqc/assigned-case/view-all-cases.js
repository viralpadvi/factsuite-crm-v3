...
function loadAllAssignedCases(){
	$.ajax({
		type: "POST",
	  	url: base_url+"outPutQc/getAllAssignedCases", 
	  	dataType: "json",
      ...
      if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) { 
        	...
        	if (data[i].is_submitted == '0') {
                ...
                // classStatus = 'pending'
                fontAwsom = '<i class="fa fa-check">'
                status = '<span class="text-info">Verified by Analyst or Specialist</span>';
        	...
        	// html += '<td class="">'+j+'</td>';
            	...
            	html += '<td class="text-center"><a href="'+base_url+'factsuite-outputqc/assigned-view-case-detail/'+data[i].candidate_id+'" ><i class="fa fa-eye"></i></a></div></td>';
           html += '</tr>';
