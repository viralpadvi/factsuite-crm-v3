...
$("#add-right-to-work").on('click',function(){
	right_to_work()
});
...
function right_to_work(){
	...
	$(".period-of-stay").each(function() { 
		remark_period_of_stay.push({remark_period_of_stay : $(this).val()}); 
	}); 

	var remark_gender = [];
	$(".gender").each(function() { 
		remark_gender.push({remark_gender : $(this).val()}); 
	}); 
...
$("#add-right-to-work").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');

		$.ajax({
            type: "POST",
              url: base_url+"outPutQc/update_remarks_right_to_work",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
              success: function(data) {
				...
				if (data.status == '1') {
              	...
              	$("#add-right-to-work").html("Confirm")
              }
        });
	}else{
