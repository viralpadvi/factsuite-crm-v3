...
function handleFileSelect_candidate_aadhar(e){ 
    ...
    if (files.length <= max_client_document_select) {
    	$("#selected-civil-docs-li"+number).html('');
        $("#civil-upoad-docs-error-msg-div"+number).html('');
        if (files[0].size <= client_document_size) {
	        ...
	        for (var i = 0; i < files.length; i++) {
	            ...
	            // alert(number)
	                ...
	                obj.push(files[i]);
	                $("#civil-upoad-docs-error-msg-div"+number).append(html);
	        	}

	        }
	        candidate_aadhar.push({[number]:obj}); 
	    } else {
	    	$("#selected-civil-docs-li"+number).html('<div class="col-md-12"><span class="text-danger error-msg-small">Document size should be of max 20 Mb.</span></div>');
			$('#client-documents'+number).val('');
	    }
    } else {
        $("#selected-civil-docs-li"+number).html('<span class="text-danger error-msg-small">Please select a max of '+max_client_document_select+' files</span>');
    }
}
...
$("#add-civil-check").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');

		$.ajax({
            type: "POST",
              url: base_url+"outPutQc/update_remarks_candidate_civil_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
