...
$("#add-court-record").on('click',function(){
  ...
  // consolel.log(JSON.stringify(candidateInfo))
  bankruptcy_check()
...
function bankruptcy_check(){
  ...
  if(candidateInfo['insuff_remarks'] != null || candidateInfo['insuff_remarks'] != ""){
...
$("#add-court-record").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
  ...
  $("#wait-message").html("<span class='text-warning'>Please wait while we are updating the data.</span>");
    $.ajax({
            type: "POST",
              url: base_url+"outPutQc/update_remarks_bankruptcy_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
