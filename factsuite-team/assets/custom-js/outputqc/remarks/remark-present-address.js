...
$("#submit-present-address").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
	
	$.ajax({
        type: "POST",
          url: base_url+"outPutQc/update_remarks_candidate_present_address",
          data:formdata,
          dataType: 'json',
          contentType: false,
          processData: false,
          success: function(data) {
			if (data.status == '1') {
          	...
          	$("#submit-present-address").html("Confirm")
          }
        });
	// }else{
