...
function cv_check(){
	...
	$(".insuff_remarks").each(function(){
		...
		// if ($(this).val() !='' && $(this).val() !=null) {
			...
			// console.log(JSON.stringify(insuff_remarks))
		// }
	}); 
	...
	$(".progress_remarks").each(function(){
		...
		// if ($(this).val() !='' && $(this).val() !=null) {
			...
			// console.log(JSON.stringify(progress_remarks))
		// }
	}); 
	...
	$(".verification_remarks").each(function(){
		...
		// if ($(this).val() !='' && $(this).val() !=null) {
			...
			// console.log(JSON.stringify(verification_remarks))
		// }
	}); 
	// var closure_remarks = [];
	...
	$(".closure_remarks").each(function(){
		...
		// if ($(this).val() !='' && $(this).val() !=null) {
			...
			// console.log(JSON.stringify(closure_remarks))
		// }
	}); 

	// change Start
	
	var candidate_full_name = $('.candidate_full_name').val()
	...
	var employement_duration = $('.employement_duration').val()	 

	var designation_held = $('.designation_held').val()
	 
	var complete_emp_info = $('.complete_emp_info').val()
	...
	// formdata.append('url',1);candidate_full_name
	...
	formdata.append('candidate_full_name',candidate_full_name);
	formdata.append('contact_number',contact_number);
	formdata.append('candidate_address',candidate_address);
	formdata.append('education_details',education_details);
	formdata.append('employement_duration',employement_duration);
	formdata.append('designation_held',designation_held);
	formdata.append('complete_emp_info',complete_emp_info);
...
$("#add-court-record").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
	...
	$("#wait-message").html("<span class='text-warning'>Please wait while we are updating the data.</span>");
		$.ajax({
            type: "POST",
              url: base_url+"outPutQc/update_remarks_cv_check",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
