...
class TeamModel extends CI_Model
{
	...
	function insert_bulk_case($bulk_case){
		...
		if ($this->db->insert_batch('email_details',$bulk_case)) {
	...
	function get_team_all_details($team_id=''){
		...
		if ($team_id != '') {
			...
			$row['assigned_segments'] = $result['assigned_segments'];
			$row['team_employee_password'] = $result['team_employee_password'];
			$row['role'] = $result['role'];
			$row['skills'] = $result['skills'];  
			$row['approval_access_level'] = $result['approval_access_level'];
			$row['approval_list'] = $result['approval_list'];
			$row['approver_status'] = $result['approver_status']; 
			$row['present_address_iverify_status'] = isset($result['present_address_iverify_status'])?$result['present_address_iverify_status']:'';
			$row['present_address_pv_status'] = isset($result['present_address_pv_status'])?$result['present_address_pv_status']:'';
			$row['permanent_address_iverify_status'] = isset($result['permanent_address_iverify_status'])?$result['permanent_address_iverify_status']:'';
			$row['permanent_address_pv_status'] = isset($result['permanent_address_pv_status'])?$result['permanent_address_pv_status']:'';
			$row['previous_address_iverify_status'] = isset($result['previous_address_iverify_status'])?$result['previous_address_iverify_status']:'';
			$row['previous_address_pv_status'] = isset($result['previous_address_pv_status'])?$result['previous_address_pv_status']:''; 
			...
			foreach ($result as $key => $value) {
				...
				$row['assigned_segments'] = $value['assigned_segments'];
				$row['team_employee_password'] = $value['team_employee_password'];
				$row['role'] = $value['role'];
				$row['skills'] = $value['skills'];
				// $row['present_address_iverify_status'] = $result['present_address_iverify_status'];
				// $row['present_address_pv_status'] = $result['present_address_pv_status'];
				// $row['permanent_address_iverify_status'] = $result['permanent_address_iverify_status'];
				// $row['permanent_address_pv_status'] = $result['permanent_address_pv_status'];
				// $row['previous_address_iverify_status'] = $result['previous_address_iverify_status'];
				// $row['previous_address_pv_status'] = $result['previous_address_pv_status'];
	...
	function insert_team() {
		...
		$team_data = array(
			...
			'approver_status'=>$this->input->post('approver'),
			'approval_list'=>$this->input->post('approval_list'),
			'reporting_manager'=>$this->input->post('manager')
		); 

		if ($this->input->post('skill')!==null) {
			$team_data['skills'] = $this->input->post('skill');
		}

		if ($this->input->post('selected_segments') != null) {
			$team_data['assigned_segments'] = $this->input->post('selected_segments');
		}

		if ($this->input->post('level') != null) {
			$team_data['approval_access_level'] = $this->input->post('level');
		}

		if ($this->input->post('present_address_component') == 1) {
			if (in_array(1, explode(',', $this->input->post('present_address_component_checked_types')))) {
				$team_data['present_address_iverify_status'] = 1;
			}

			if (in_array(2, explode(',', $this->input->post('present_address_component_checked_types')))) {
				$team_data['present_address_pv_status'] = 1;
			}
		}

		if ($this->input->post('permanent_address_component') == 1) {
			if (in_array(1, explode(',', $this->input->post('permanent_address_component_checked_types')))) {
				$team_data['permanent_address_iverify_status'] = 1;
			}

			if (in_array(2, explode(',', $this->input->post('permanent_address_component_checked_types')))) {
				$team_data['permanent_address_pv_status'] = 1;
			}
		}

		if ($this->input->post('previous_address_component') == 1) {
			if (in_array(1, explode(',', $this->input->post('previous_address_component_checked_types')))) {
				$team_data['previous_address_iverify_status'] = 1;
			}

			if (in_array(2, explode(',', $this->input->post('previous_address_component_checked_types')))) {
				$team_data['previous_address_pv_status'] = 1;
			}
		}
...
/*
			...
			if ($this->input->post('skill')!==null) {
				$team_log_data['skills'] = $this->input->post('skill');
			}

			if ($this->input->post('selected_segments') != null) {
				$team_log_data['assigned_segments'] = $this->input->post('selected_segments');
			}

			if ($this->input->post('present_address_component') == 1) {
				if (in_array(1, explode(',', $this->input->post('present_address_component_checked_types')))) {
					$team_log_data['present_address_iverify_status'] = 1;
				}

				if (in_array(2, explode(',', $this->input->post('present_address_component_checked_types')))) {
					$team_log_data['present_address_pv_status'] = 1;
				}
			}

			if ($this->input->post('permanent_address_component') == 1) {
				if (in_array(1, explode(',', $this->input->post('permanent_address_component_checked_types')))) {
					$team_log_data['permanent_address_iverify_status'] = 1;
				}

				if (in_array(2, explode(',', $this->input->post('permanent_address_component_checked_types')))) {
					$team_log_data['permanent_address_pv_status'] = 1;
				}
			}

			if ($this->input->post('previous_address_component') == 1) {
				if (in_array(1, explode(',', $this->input->post('previous_address_component_checked_types')))) {
					$team_log_data['previous_address_iverify_status'] = 1;
				}

				if (in_array(2, explode(',', $this->input->post('previous_address_component_checked_types')))) {
					$team_log_data['previous_address_pv_status'] = 1;
				}
			}
	...
	function update_team() {
		...
		$team_data['present_address_iverify_status'] = $team_data['present_address_pv_status'] = $team_data['permanent_address_iverify_status'] = $team_data['permanent_address_pv_status'] = $team_data['previous_address_iverify_status'] = $team_data['previous_address_pv_status'] = 0;
		...
		if ($this->db->update('team_employee',$team_data)) {
			...
			$team_log_data['present_address_iverify_status'] = $team_log_data['present_address_pv_status'] = $team_log_data['permanent_address_iverify_status'] = $team_log_data['permanent_address_pv_status'] = $team_log_data['previous_address_iverify_status'] = $team_log_data['previous_address_pv_status'] = 0;
			if ($this->input->post('present_address_component') == 1) {
				if (in_array(1, explode(',', $this->input->post('present_address_component_checked_types')))) {
					$team_log_data['present_address_iverify_status'] = 1;
				}
