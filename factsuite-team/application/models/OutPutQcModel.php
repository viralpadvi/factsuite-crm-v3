...
class OutPutQcModel extends CI_Model
{
	...
	function getAllAssignedCases() {
		...
		if($outputqc_info != null){
			return $this->db->order_by('candidate.candidate_id','DESC')->select("candidate.*,tbl_client.client_name,packages.package_name")->from("candidate")->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->join('packages','candidate.package_name = packages.package_id','left')->where('candidate.assigned_outputqc_id',$outputqc_info['team_id'])->get()->result_array();
		}else{
			return $this->db->order_by('candidate.candidate_id','DESC')->select("candidate.*,tbl_client.client_name,packages.package_name")->from("candidate")->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->join('packages','candidate.package_name = packages.package_id','left')->get()->result_array();
		}
	...
	function getAllAssignedCases_() {
		...
		$candidateData = array();
		if($outputqc_info != null){
			$candidateData =  $this->db->order_by('candidate.candidate_id','DESC')->select("candidate.*,tbl_client.client_name,packages.package_name")->from("candidate")->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->join('packages','candidate.package_name = packages.package_id','left')->where('candidate.assigned_outputqc_id',$outputqc_info['team_id'])->get()->result_array();
		}else{
			$candidateData =  $this->db->order_by('candidate.candidate_id','DESC')->select("candidate.*,tbl_client.client_name,packages.package_name")->from("candidate")->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->join('packages','candidate.package_name = packages.package_id','left')->get()->result_array();
		}
	...
	function getSingleAssignedCaseDetails($candidate_id) {
 		$result = $this->db->where('candidate_id',$candidate_id)->select("candidate.*,tbl_client.client_name,packages.package_name,tv_or_ebgv")->from("candidate")->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->join('packages','candidate.package_name = packages.package_id','left')->get()->row_array();
	...
	function genrateReportStatus() {
		...
		if($candidateinfo != null) {
			...
			$variable_array = array(
				...
				'candidate_details_added_from' => $candidateinfo['candidate_details_added_from'],
				'contact_us_details' => $contact_us_details
			);
			$client_email_message = $this->branding_Emails_Model->send_case_completed_mail_to_client($variable_array);

			if ($candidateinfo['candidate_details_added_from'] == '0') {
				$send_email_to_user = $this->emailModel->send_mail($client['spoc_email_id'],$candidate_email_subject,$client_email_message);
				$this->smsModel->send_complete_sms($client['client_name'],$client['spoc_phone_no']);
			}

			$get_client_details = $this->db->where('client_id',$candidateinfo['client_id'])->get('tbl_client')->row_array();
			if ($get_client_details['candiate_notification_status'] == '1') {
				if ($candidateinfo['case_complition_email_sent_status'] == '0' || $candidateinfo['case_complition_sms_sent_status'] == '0') {
					$variable_array = array(
						'candidate_details' => $candidateinfo,
						'client_details' => $get_client_details
					);
					$mail_to_candidate = $this->mail_to_candidate($variable_array);
					$sms_to_candidate = $this->sms_to_candidate($variable_array);

					$update_candidate_notification_data = [];
					$date = date('Y-m-d H:s:i');
					if ($mail_to_candidate['status'] == 1) {
						$update_candidate_notification_data['case_complition_email_sent_status'] = 1;
						$update_candidate_notification_data['case_complition_email_sent_date'] = $date;
					}

					if ($sms_to_candidate['status'] == 1) {
						$update_candidate_notification_data['case_complition_sms_sent_status'] = 1;
						$update_candidate_notification_data['case_complition_sms_sent_date'] = $date;
					}

					$this->db->where('candidate_id',$candidate_id)->update('candidate',$update_candidate_notification_data);
				}
			}
			...
			// $this->analystModel->notification_action_report($candidate_data);
	...
	function mail_to_candidate($variable_array) {
		$email_subject = 'Verification Successfull';

		$email_message = '<html> ';
		$email_message .= '<head>';
		$email_message .= '</head>';
		$email_message .= '<body> ';
		$email_message .= "<p>Dear ".$variable_array['candidate_details']['first_name']." ".$variable_array['candidate_details']['last_name'].",</p>";
		$email_message .= '<p>Greetings from Factsuite!!</p>';
		$email_message .= '<p>Your verification is successfull.</p>';
		$email_message .= '<p><b>Yours sincerely,<br>';
		$email_message .= 'Team FactSuite</b></p>';
		$email_message .= '</body>';
		$email_message .= '</html>';
		if($this->emailModel->send_mail($variable_array['candidate_details']['email_id'],$email_subject,$email_message)) {
			return array('status'=>'1','message'=>'Mail sent successfully.');
		} else {
			return array('status'=>'0','message'=>'Something went wrong while triggering mail. Please try again.');
		}
	}

	function sms_to_candidate($variable_array) {
		$send_complete_sms = $this->smsModel->send_complete_sms($variable_array['candidate_details']['first_name'],$variable_array['candidate_details']['phone_number']);
		if ($send_complete_sms == 200) {
			return array('status'=>'1','message'=>'SMS sent successfully.');
		} else {
			return array('status'=>'0','message'=>'Something went wrong while triggering SMS. Please try again.');
		}
	}
	...
	function update_remarks_candidate_criminal_check(){
		...
		$criminal_checks = array(
			...
			'outputqc_status_date'=>implode(',', $op_date),
	...
	function update_remarks_candidate_civil_check(){
		...
		$civil_check = array(
			...
			$componentId = $this->utilModel->getComponentId('civil_check');
			$userId = $this->session->userdata('logged-in-outputqc');
			$userId = $userId['team_id'];
			$this->notificationModel->notificationCreate('civil_check','civil_check_id',$civil_check_id,$componentId,$userId);
			// exit();
			$this->db->where('civil_check_id',$civil_check_id);
		if ($this->db->update('civil_check',$civil_check)) { 
			
			$civil_check['civil_check_id'] = $civil_check_id;			
	...
	function update_remarks_candidate_permanent_address(){
		...
		$permanent_address = array(
			...
			'outputqc_status_date'=>date('d-m-Y H:i:s'),
			'ouputqc_comment'=>$this->input->post('ouputQcComment')
			);
		...
		if ($this->db->update('permanent_address',$permanent_address)) {
			...
			$permanent_address['candidate_id'] = $this->input->post('candidate_id');
