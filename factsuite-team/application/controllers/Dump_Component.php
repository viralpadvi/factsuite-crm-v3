...
$is_submitted = '';
  ...
  $status = '';
    ...
    function export_cases_assigned_to_vendor() {
        ...
        $where = '';
        if ($this->input->post('duration') == 'today') {
            $where = "  date(assign_case_to_vendor.created_date) = CURDATE()";
        } else if($this->input->post('duration') == 'week') {
            $where = "  date(assign_case_to_vendor.created_date) BETWEEN CURDATE() - INTERVAL 7 DAY AND CURDATE()";
        } else if($this->input->post('duration') == 'month') {
            $where = "  date(assign_case_to_vendor.created_date) BETWEEN CURDATE() - INTERVAL 1 MONTH AND CURDATE()";
        } else if($this->input->post('duration') == 'year') {
            $where = "  date(assign_case_to_vendor.created_date) BETWEEN CURDATE() - INTERVAL 1 YEAR AND CURDATE()";
        } else if($this->input->post('duration') == 'between') {
            $where = "  date(assign_case_to_vendor.created_date) BETWEEN  '".$_POST['from']."' AND '".$_POST['to']."'";
        }else{
             // $where = "  date(assign_case_to_vendor.created_date) BETWEEN CURDATE() - INTERVAL 1 MONTH AND CURDATE()";
        }
        ...
        $data = $this->db->select('vendor.vendor_name,assign_case_to_vendor.*,assign_case_to_vendor.created_date AS assigned_to_vendor_date, candidate.*')->from('assign_case_to_vendor')->join('vendor','assign_case_to_vendor.vendor_id = vendor.vendor_id','left')->join('candidate','assign_case_to_vendor.case_id = candidate.candidate_id','left')->order_by('vendor_id','DESC')->get()->result_array();
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($cases as $k => $val) { 
                 if ($value['index_no'] == $val['position'] && $value['component_id'] == $val['component_id']) { 
                $status = '';
                if ($val['analyst_status'] == '0') {             
                    $status = 'Not initiated';
                } else if ($val['analyst_status'] == '1') {
                  ...
                  $is_submitted = '';
                    ...
                    $team_name = $this->db->where('team_id',$value['assignment_team_id'])->get('team_employee')->row_array();
                    ...
                    $candidate_data[$val['candidate_id']] = array( 
                        ...
                        'date_of_birth' => $val['date_of_birth'],
                        'client_name' => $val['client_name'],
                        'assigned_team_name' => isset($team_name['first_name'])?$team_name['last_name']:'-',
                        'vendor_name' => $value['vendor_name'],
                        'csm' => $val['csm'],
                        'role' => isset($team_name['role'])?$team_name['role']:'',
                        'analyst_status' => $status,
                        'status' => $is_submitted,
                        'created_date' => $value['created_date'],
                        'init_date' => $val['created_date'],
                        'updated_date' => $val['updated_date'],
                        'TAT' => $val['left_tat_days'],
                        'component_name' => $val['component_name'].' '.$val['formNumber'],
                        'assigned_to_vendor_date' => $value['assigned_to_vendor_date'],
                        'case_completed_by_vendor_date' => $value['case_completed_by_vendor_date'] != '' ? $value['case_completed_by_vendor_date'] : '-'
                    );
        ...
        // $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'First Name');
        // $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Last Name');
        // $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Case Status');
        // $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Component Name'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Component Status'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Vendor initiation Date'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Vendor Closure Date'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('L1', 'Analyst / Specialist'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('M1', 'Vendor Names'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('N1', 'Vendor re-initiation date'); 
        // $objPHPExcel->getActiveSheet()->SetCellValue('O1', 'Vendor re-initiation closure date'); 
        ...
        //                 ->getColor()
        //                 ->setRGB('000000'); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, (++$init)); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $val['candidate_id']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $this->utilModel->get_actual_date_formate($val['init_date'])); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $val['client_name']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $val['first_name']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $val['last_name']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $val['status']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $val['component_name']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $val['analyst_status']);  
        //             $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $this->utilModel->get_actual_date_formate($val['created_date'])); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $this->utilModel->get_actual_date_formate($val['updated_date'])); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('L' . $rowCount, $val['assigned_team_name']); 
        //             $objPHPExcel->getActiveSheet()->SetCellValue('M' . $rowCount, $val['vendor_name']); 
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'Client Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'Candidate Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Employee ID');
        $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Component Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Component Status');  
        $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Assigned To Vendor Name'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'Assigned To Vendor Date'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Vendor Completion Date');
        ...
        if (count($candidate_data) > 0) {  
                 ...
                 foreach ($candidate_data as $k => $val) {
                    $objPHPExcel->getActiveSheet()
                        ...
                        ->setRGB('000000'); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, (++$init)); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $this->utilModel->get_actual_date_formate($val['init_date'])); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $val['candidate_id']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $val['client_name']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $val['first_name']. ' '.$val['last_name']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $val['employee_id']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $val['component_name']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $val['analyst_status']);  
                    $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $val['vendor_name']); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $this->utilModel->get_actual_date_formate($val['assigned_to_vendor_date'])); 
                    $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $val['case_completed_by_vendor_date'] != '-' ? $this->utilModel->get_actual_date_formate($val['case_completed_by_vendor_date']) : '');
                    $rowCount++;
            }
        }
      ...
      function export_finance_report(){
        ...
        $case = $this->db->query('SELECT * FROM candidate '.$where.'  ORDER BY candidate_id DESC')->result_array();
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[12] .'1', 'Initiated Data');  
            $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[13] .'1', 'Completed Date');  
            $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[14] .'1', 'TAT Days');  
            ...
            $alphabet = $this->utilModel->return_excel_val(); 
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue( $alphabet[$n++] . $rowCount, $val['case_submitted_date']);
                $objPHPExcel->getActiveSheet()->SetCellValue( $alphabet[$n++] . $rowCount, $val['completed_date']);
                $objPHPExcel->getActiveSheet()->SetCellValue( $alphabet[$n++] . $rowCount, $val['tat']);
    ...
    function get_order_details() {
        ...
        $get_website_services = $this->db->get('website_services')->result_array();
        $client_array = array();
        if (count($result) > 0) {
            foreach ($result as $key => $value) {
                ...
                $row['hsn_sac_number'] = '-';
                foreach ($get_website_services as $key1 => $value1) {
                    if ($value1['service_id'] == $package_component['service_id']) {
                        $row['hsn_sac_number'] = $value1['service_hsn_sac_number'];
                        break;
                    }
                }
                ...
                $row['order_sub_total'] = $total_value;
                $row['service_id'] = $package_component['service_id'];
                ...
                if ($package_component['selected_tat_details']['price'] > 0) {
                    $row['surcharge_amount'] = (floatval($total_value) * (floatval($package_component['selected_tat_details']['price']) / 100));
                    if ($value['surcharge_waved_off_status'] == 1) {
                        $row['discount_amount'] = $row['surcharge_amount'];       
                    }
                }
                ...
                $row['sgst'] = '-';
                $row['igst'] = '-';
                $row['gst_amount'] = (floatval($total_value) * (floatval($value['gst_applied']) / 100));
                if (in_array(strtolower($value['gst_state_name']), $this->config->item('fs_home_states'))) {
                    $gst_percentage = floatval($value['gst_applied']) / 2;
                    $row['cgst'] = $gst_percentage;
                    $row['sgst'] = $gst_percentage;
                } else {
                    $row['igst'] = $value['gst_applied'];
                }
                $row['client_state'] = (isset($client['client_state']) && $client['client_state'] != '') ? $client['client_state'] : '-';
                $row['client_id'] = $value['user_id'];
                $row['invoice_no'] = $value['cart_id']; 
                $row['date'] = explode(' ',$value['package_purchased_date'])[0];
                $row['qty'] = $package_component['no_of_candidates'];
                $row['service_id'] = $package_component['service_id'];
                $row['invoice_total'] = floatval($total_value) + floatval($row['surcharge_amount']) - floatval($row['discount_amount']) + floatval($row['gst_amount']);
        ...
        $n = 0;
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Sr.no.');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Invoice Number');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Invoice Date');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Customer Name');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'GST Nuber');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Place of Supply');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Address');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Currency');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Exchange Rate');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Item Name');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Item Desc');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Item Type');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'HSN/SAC');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Quantity');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Usage unit');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Subtotal');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Surcharge Amount');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Discount Amount');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Shipping Charge');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'CGST(%)');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'SGST(%)');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'IGST(%)');
        $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] .'1', 'Invoice Total');
        ...
        if (count($client_array) > 0) {  
            ...
            foreach ($client_array as $k => $val) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, '-'); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, '-'); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['hsn_sac_number']); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['qty']); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, '-'); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['order_sub_total']); 
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['surcharge_amount']);
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['discount_amount']);
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, '0');
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['cgst']);
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['sgst']);
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['igst']);
                $objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$n++] . $rowCount, $val['invoice_total']);
