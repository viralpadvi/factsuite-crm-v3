...
<!-- <script src="http://ajax.aspnetcdn.com/ajax/jquery.validate/1.11.1/jquery.validate.min.js"></script> -->
  ...
  if (!$this->session->userdata('logged-in-specialist')) {
   ...
   $sessionData= $this->session->userdata('logged-in-specialist');?>
  <title>Fact Suite - Specialist</title>
</head>
<body>
<!--Header-->
<!-- <header>
   <div class="header-cntr">
      ...
      <div class="header-rgt">
         <ul>
            ...
            <li class="candate"><img src="<?php echo base_url(); ?>assets/admin/images/candidate.png"/>Specialist: <?php echo $sessionData['first_name']?></li>
            <li><a href="<?php echo $this->config->item('my_base_url')?>factsuite-specialist/specialist-logout"><i class="fa fa-power-off"></i></li>
         </ul>
      </div>
      <div class="clr"></div>
   </div>
            ...
            <div class="hdr-mn">
               <ul>
                  <li class="nav-item dropdown ml-1" title="Internal Chat">
                     <a class="nav-link" href="<?php echo $this->config->item('my_base_url')?>factsuite-specialist/internal-chat">
                  ...
                  <li class="nav-item dropdown ml-1" title="Qc Error Notification" id="qc_error_notification">
                     <a class="nav-link" data-toggle="dropdown" href="#">
                  ...
                  <li>
                     <a href="#" class="candate"><span></span><?php echo $sessionData['first_name'];?></a>
                  </li>
                  <li>
                     <a href="<?php echo $this->config->item('my_base_url')?>factsuite-specialist/specialist-logout" class="lg-out"><i class="fa fa-power-off" aria-hidden="true"></i></a>
                  </li>
...
<script>
  ...
  var team_id ="<?php echo $sessionData['team_id']?>";
            ...
            if(data.length > 0){
               ...
               for (var i = 0; i < data.length; i++) { 
                  ...
                  html += '<a href="'+base_url+'factsuite-analyst/component-detail/'+data[i].case_id+'/'+data[i].component_id+'/'+data[i].index_number+'" onclick="clear_the_notification('+data[i].approval_id+')" class="dropdown-item" target="_blank">'
                     html += '<i class="fa fa-id-card-o noti-icon-color mr-2"></i>Approval id:'+data[i].approval_id+' Status : '+approve+' ('+data[i].approved_by_role+')';
   ...
   function get_clarification_comments_notification() {
      $.ajax({
         type:'post',
         url:base_url+'factsuite-specialist/get-new-client-clarification-comments-notifications',
         data : {
         ...
         success:function(data) {
            ...
            if(data.all_clarification_comments.length > 0) {
               ...
               for (var i = 0; i < all_clarification_comments.length; i++) {
                  html += '<a href="'+base_url+'factsuite-specialist/component-detail/'+all_clarification_comments[i].candidate_id+'/'+all_clarification_comments[i].component_id+'/'+all_clarification_comments[i].component_index+'?view_client_clarification='+all_clarification_comments[i].user_filled_details_component_client_clarification_id+'" class="dropdown-item">';
         ...
         success:function(data){
            ...
            if(data.length > 0){
               ...
               for (var i = 0; i < data.length; i++) {                   
                  ...
                  html += '<a href="'+base_url+'factsuite-specialist/component-detail/'+data[i].case_id+'/'+data[i].component_id+'/'+data[i].case_index+'" class="dropdown-item" target="_blank">'
   ...
   function get_ticket_notifications() {
      $.ajax({
         type: 'post',
         url: base_url+'factsuite-specialist/get-ticket-notifications',
         data: {
            verify_specialist_request : 1
         },
         dataType: 'json',
         success: function(data) {
            ...
            if (data.all_ticket_notifications.length > 0) {
               ...
               for (var i = 0; i < data.all_ticket_notifications.length; i++) {
                  html += '<a href="'+base_url+'factsuite-specialist/tickets-assigned-to-me?tkt_id='+all_ticket_notifications[i].ticket_id+'" class="dropdown-item"><i class="fa fa-id-card-o noti-icon-color mr-2"></i>';
