            ...
            <div class="hdr-mn">
               <ul>
                  <li class="nav-item dropdown ml-1" title="Internal Chat">
                     <a class="nav-link" href="<?php echo $this->config->item('my_base_url')?>factsuite-csm/internal-chat">
                  ...
                  <li>
                     <a href="#" class="candate"><span></span><?php echo isset($userData['first_name'])?$userData['first_name']:"CSM"?></a>
                  </li>
                  <li>
                     <a href="<?php echo $this->config->item('my_base_url')?>factsuite-csm/csm-logout" class="lg-out"><i class="fa fa-power-off" aria-hidden="true"></i></a>
                  </li>
         ...
         success:function(data){
            ...
            if(data.length > 0){
               ...
               for (var i = 0; i < data.length; i++) { 
                  ...
                  html += '<a href="'+base_url+'factsuite-csm/approval-level-mechanism" onclick="clear_the_notification('+data[i].approval_id+')" class="dropdown-item" target="_blank">'
...
<script>
         ...
         success:function(data){
            ...
            if(data.length > 0){
               ...
               for (var i = 0; i < data.length; i++) { 
                  ...
                  html += '<a href="'+base_url+'factsuite-csm/client-adding-deletion-approval-mechanism" onclick="clear_the_notification('+data[i].approval_id+')" class="dropdown-item" target="_blank">'
                     html += '<i class="fa fa-id-card-o noti-icon-color mr-2"></i>Approval id:'+data[i].approval_id+' Status : '+status+' ('+data[i].approved_by_role+')';
   ...
   function get_escalatory_cases_notifications() {
      $.ajax({
         type: 'post',
         url: base_url+'factsuite-csm/get-escalatory-cases-notifications',
         data: {
         ...
         success: function(data) {
            ...
            if (data.all_escalatory_cases.length > 0) {
               ...
               var all_case = data.all_escalatory_cases;
                html += '<a href="javascript:void(0)" class="dropdown-item">';
               html += '<i class="fas fa-envelope mr-2"></i> '+data.all_escalatory_cases.length+' Notifications';
               html += '</a>';
               for (var i = 0; i < data.all_escalatory_cases.length; i++) {
                  html += '<a href="'+base_url+'factsuite-csm/view-case-detail/'+all_case[i].candidate_id+'" class="dropdown-item"><i class="fa fa-id-card-o noti-icon-color mr-2"></i>';
   ...
   function get_ticket_notifications() {
      $.ajax({
         type: 'post',
         url: base_url+'factsuite-csm/get-ticket-notifications',
         data: {
            verify_csm_request : 1
         },
         dataType: 'json',
         success: function(data) {
            ...
            if (data.all_ticket_notifications.length > 0) {
               ...
               for (var i = 0; i < data.all_ticket_notifications.length; i++) {
                  html += '<a href="'+base_url+'factsuite-csm/tickets-assigned-to-me?tkt_id='+all_ticket_notifications[i].ticket_id+'" class="dropdown-item"><i class="fa fa-id-card-o noti-icon-color mr-2"></i>';
