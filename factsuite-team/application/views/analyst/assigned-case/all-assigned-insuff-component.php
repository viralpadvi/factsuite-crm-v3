  ...
  <div class="pg-cnt">
     <div id="FS-candidate-cnt" class="FS-candidate-cnt-admin"> 
        <h3>View All Insuff Component</h3>
          ...
          <table class="datatable table table-striped">
            ...
            <tbody class="tbody-datatable" id="get-insuff-component-data-1"> 
               <!-- dynamic data table -->
                <?php 
                 $i = 1; 
                    if (count($case) > 0) {
                        foreach ($case as $key => $value) {
                            if ($value['analyst_status'] != '4') { 
                    ...
                    if ($value['analyst_status'] == '0' || $value['analyst_status'] == '1' || $value['analyst_status'] == '11') {
                     ...
                     $first_name = isset($value['candidate_detail']['first_name'])?$value['candidate_detail']['first_name']:'-';
                     $last_name = isset($value['candidate_detail']['last_name'])?$value['candidate_detail']['last_name']:'-';
                     $candidate_name = $first_name." ".$last_name;
                     $code = isset($value['candidate_detail']['country_code'])?$value['candidate_detail']['country_code']:'+91';
                     $phone_number = isset($value['candidate_detail']['phone_number'])?$value['candidate_detail']['phone_number']:'-';
                            ?>
                            <tr>
                                <td><?php echo $i++; ?></td>
...
<!-- <script src="<?php echo base_url() ?>assets/custom-js/analyst/assigned-case/view-insuff-component-detail.js"></script> -->
...
<script>
  // loadAllInsuffComponent('<?php echo $sessionData['team_id'];?>')
</script>
