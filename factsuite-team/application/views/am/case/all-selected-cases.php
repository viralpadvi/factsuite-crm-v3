          ...
          <table class="table-fixed  datatable table table-striped">
            <thead class="table-fixed-thead thead-bd-color">
              <tr>
                ...
                <th>Re-Assignment Analyst/Specialist</th>
                <!-- <th>View Details</th> -->
                <th>View Details</th>
                <!-- <th>Action</th> -->
                <!-- <th>Insufficiency</th>  
            ...
            <tbody class="table-fixed-tbody tbody-datatable" id="get-case-data-1"> 
                <?php 
                 $i = 1; 
                    ...
                    if (in_array($value['analyst_status'],['0','1'])) { 
                             $inputQcStatus= '';
                    $analystQcStatus= '';
                    ...
                    if ($value['analyst_status'] == '0' || $value['analyst_status'] == '1' || $value['analyst_status'] == '11') {
                     ...
                     $override_team_arg = $value['candidate_id'].','.$value['component_id'].',\''.$value['component_name'].'\','.$value['index'].','.$key;
                     $arg = $value['candidate_id'].'/'.$value['component_id'].'/'.$value['index'];
                     $form_number = $value['index'] + 1;
                      $first = isset($value['candidate_detail']['first_name'])?$value['candidate_detail']['first_name']:'';
                     $last = isset($value['candidate_detail']['last_name'])?$value['candidate_detail']['last_name']:'';
                     $candidate_name = $first." ".$last;

                     if ($value['analyst_status'] != '3') { 
                            ?>
                            <tr>
                                <td><?php echo $i++; ?></td>
                                ...
                                <td><?php echo $candidate_name; ?></td>
                                 <td><?php echo isset($value['candidate_detail']['client_name'])?$value['candidate_detail']['client_name']:''; ?></td>
                                <td><?php echo isset($value['candidate_detail']['phone_number'])?$value['candidate_detail']['phone_number']:''; ?></td>
                                <td><?php echo $analystQcStatus; ?></td>

                                <?php
                              $assigned_team_id =  isset($value['assigned_team_id'])?$value['assigned_team_id']:'-';
                                 if($assigned_team_id != '-'){ ?>
                    <td id="emp_names_<?php echo $key; ?>" >
                        <select class="sel-allcase" id="override_team_<?php echo $key; ?>" onchange="override_team(<?php echo $override_team_arg; ?>,this.id)"> 
                            <?php for ($e = 0; $e < count($value['emp_data_analyst']); $e++) {  
                                if($value['assigned_team_id'] == $value['emp_data_analyst'][$e]['team_id']){ ?>
                                    <option selected value="<?php echo $value['emp_data_analyst'][$e]['team_id']; ?> "><?php echo $value['emp_data_analyst'][$e]['first_name']; ?> &nbsp;(<?php echo $value['emp_data_analyst'][$e]['role']; ?> )</option>                            
                               <?php }else{ ?>
                                    <option value="<?php echo $value['emp_data_analyst'][$e]['team_id']; ?> "><?php echo $value['emp_data_analyst'][$e]['first_name']; ?> &nbsp;(<?php echo $value['emp_data_analyst'][$e]['role']; ?> )</option>                           
                              <?php  }
                            }   ?>                                         
                        </select> 
                    </td> 
                ...
                <?php  } ?>
                                ...
                                <td><a href="<?php echo $this->config->item('my_base_url'); ?>factsuite-am/component-detail/<?php echo $arg; ?>" class="app-btn">View <i class="fas fa-angle-right"></i></a></td>
                            </tr>
...
<script src="<?php echo base_url() ?>assets/custom-js/am/assigned-case/view-all-cases.js"></script>
<script src="<?php echo base_url() ?>assets/custom-js/am/assigned-case/view-single-case.js"></script>
