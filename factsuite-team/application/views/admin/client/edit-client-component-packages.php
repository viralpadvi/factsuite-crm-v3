  ...
  <div class="pg-cnt">
     <div id="FS-candidate-cnt" class="FS-candidate-cnt pt-3">
        <!--Add Client Content-->
         <div class=""> 
           <a href="<?php echo $this->config->item('my_base_url')?>factsuite-admin/edit-select-package-component-client/<?php echo $client_id; ?>" class="btn bg-blu btn-submit-cancel text-white"><i class="fas fa-arrow-left"></i> Back</a>
        </div>
        <div class="add-client-bx">
           ...
           <div id="display-component-details">
            ...
            // print_r($client['packages']);
             $pkg_comp = array();
             $total_comp = array();
           $pkg = $this->db->where_in('package_id',explode(',',$client['packages']))->get('packages')->result_array();
           foreach ($pkg as $p => $pk) { 
               $pkg_comp[$pk['package_id']] = $pk['component_ids'];  
               if ($p ==0) {
                  $total_comp = explode(',', $pk['component_ids']);
               }else{
                 $total_comp = array_merge($total_comp,explode(',', $pk['component_ids']));
               }
           }
           

           ?>
           <input type="hidden" id="valid_comp" value="<?php echo implode(',', $total_comp)?>">
           <?php 
              ...
              $comp_onent = isset($client['package_components'])?$client['package_components']:'';
                ...
                if (count(isset($package_component)?$package_component:array()) > 0) {
                  foreach ($package_component as $key => $com) {
                   ...
                   $pack_name = $this->packageModel->get_package_details($com['package_id']);
                   ...
                   if (in_array($com['component_id'],explode(',', isset($pkg_comp[$com['package_id']])?$pkg_comp[$com['package_id']]:''))) {
        ...
        <div class="row ul<?php echo $com['package_id']; ?>">
          ...
          <label class="custom-control-label" for="customCheck<?php echo $com['package_id'].'_'.$com['component_id']; ?>"><?php echo $thresold[$this->config->item('show_component_name')]; ?> (<?php echo isset($pack_name['package_name'])?$pack_name['package_name']:''; ?>)</label>
          </div>
          </div>
          <div class="col-md-4 mt-1">
          ...
          if ($com['component_id'] == 3 || $com['component_id'] == 27) {
          ...
          // html +=edu;
           ...
           }else if (!in_array($com['component_id'], [3,4,7,27])) {
