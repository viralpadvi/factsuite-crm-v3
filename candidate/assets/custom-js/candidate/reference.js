...
j++;
 ...
 $("#add_reference").on('click',function(){
	...
	if (name.length > 0 &&
company_name.length == name.length &&
designation.length == name.length &&
contact.length == name.length &&
email.length == name.length &&
association.length == name.length &&
start_date.length == name.length &&
end_date.length == name.length ) {
...
$("#add_reference").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
	...
	$.ajax({
        type: "POST",
          url: base_url+"candidate/update_candidate_reference",
          data:formdata,
          dataType: 'json',
          contentType: false,
          processData: false,
...
function valid_name(id=''){
	...
	function valid_contact(id){
		var manager_contact = $("#contact"+id).val();
			...
			input_is_invalid("#contact"+id)
			$("#contact-error"+id).html("<span class='text-danger error-msg-small'>Please enter a valid contact number</span>");
		}
	}
	...
	function valid_association(id){
		...
		if (association !='') {
			...
			$("#association-error"+id).html("<span class='text-danger error-msg-small'>Please Enter Valid association</span>");
		}
	}
	function valid_timepicker(id){
		...
		if (timepicker !='') {
			...
			$("#timepicker-error"+id).html("<span class='text-danger error-msg-small'>Please Enter Valid timepicker</span>");
		}
		validate_start_and_end_time(id);
	}
	function valid_timepicker1(id){
		...
		if (timepicker1 !='') {
			...
			$("#timepicker1-error"+id).html("<span class='text-danger error-msg-small'>Please Enter Valid timepicker</span>");
		}
		validate_start_and_end_time(id);
	}

	function validate_start_and_end_time(id) {
		var timepicker1 = $('#timepicker'+id).val(),
      timepicker2 = $('#timepicker1'+id).val();
    if(timepicker1 != timepicker2) {
      $('#timepicker1-error'+id).html('');
      return 1;
    } else {
      $('#timepicker1-error'+id).html('<span class="text-danger error-msg-small">Start time and End time cannot be same.</span>');
      $('#timepicker1'+id).val('');
      return 0;
    }
	}