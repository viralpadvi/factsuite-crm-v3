...
$("#candidate-gsa").on('click',function(){ 
	...
	formdata.append('url',33);
	formdata.append('first_name',first_name);
	formdata.append('last_name',last_name); 
	formdata.append('link_request_from',link_request_from);
	if (gsa_id !='' && gsa_id !=null) {
		formdata.append('gsa_id',gsa_id); 
	}
$("#candidate-gsa").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');

	$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
	$.ajax({
    type: "POST",
		url: base_url+"candidate/update_candidate_gsa",
		data:formdata,
		dataType: 'json',
		contentType: false,
		processData: false,
		success: function(data) {
			...
			$("#warning-msg").html("");
			$("#candidate-gsa").html("Save & Continue");
		}
	});
