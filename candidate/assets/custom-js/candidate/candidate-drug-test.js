...
address.length ==drug_count &&
contact_number.length ==drug_count &&
name.length ==drug_count &&
date_of_birth.length ==drug_count &&
father_name.length ==drug_count &&
...
function valid_father_name(id){
	...
	if (father_name != '') {
		...
		$('#father_name-error'+id).html('<span class="text-danger error-msg-small">Please enter father Name.</span>');
		input_is_invalid('#father_name'+id);
	}
}
