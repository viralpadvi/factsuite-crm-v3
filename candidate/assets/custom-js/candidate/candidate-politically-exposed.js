...
$("#candidate-politically-exposed").on('click',function(){ 

	var politically_exposed_id = $("#politically_exposed_id").val();
	var first_name = $("#first-name").val();
	...
	if (first_name !=''  && last_name !=''  && address !=''  && date_of_birth !='' ) {

	var formdata = new FormData();
	formdata.append('url',29);
	...
	if (politically_exposed_id !='' && politically_exposed_id !=null) {
		formdata.append('politically_exposed_id',politically_exposed_id); 
	}
$("#candidate-politically-exposed").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');

	$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
	$.ajax({
    type: "POST",
		url: base_url+"candidate/update_candidate_politically_exposed",
		data:formdata,
		dataType: 'json',
		contentType: false,
		processData: false,
		success: function(data) {
			...
			$("#warning-msg").html("");
			$("#candidate-politically-exposed").html("Save & Continue");
		}
	});
