...
$("#candidate-sex-offender").on('click',function(){ 

	var sex_offender_id = $("#sex_offender_id").val();
	var first_name = $("#first-name").val();
	...
	formdata.append('url',28);
	formdata.append('first_name',first_name);
	formdata.append('last_name',last_name);
	formdata.append('date_of_birth',date_of_birth); 
	formdata.append('gender',gender); 
	formdata.append('link_request_from',link_request_from);
	if (sex_offender_id !='' && sex_offender_id !=null) {
		formdata.append('sex_offender_id',sex_offender_id); 
	}
$("#candidate-sex-offender").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');

	$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
	$.ajax({
    type: "POST",
		url: base_url+"candidate/update_candidate_sex_offender",
		data:formdata,
		dataType: 'json',
		contentType: false,
		processData: false,
		success: function(data) {
			...
			$("#warning-msg").html("");
			$("#candidate-sex-offender").html("Save & Continue");
		}
	});
