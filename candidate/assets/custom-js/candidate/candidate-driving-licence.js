...
function valid_driving_licence_no(){
    ...
    if ( driving_licence_number !='' ) {
        if (driving_licence_number.length > 15) {
            $("#driving_licence_number-error").html('<span class="text-danger error-msg-small">Driving license number should be of '+15+' digits.</span>');
        ...
        $("#driving_licence_number-error").html("<span class='text-danger error-msg-small'>Please enter valid driving license number</span>");
        input_is_invalid("#driving_licence_number")
    }
}
...
var driving_licence_number = $("#driving_licence_number").val(); 
    ...
    var licence_id = $("#driving_licence_id").val();
    if (licence_id !='' && licence_id !=null) {
        ...
        if (driving_licence.length > 0 ) {
            for (var i = 0; i < driving_licence.length; i++) { 
                formdata.append('driving_licence[]',driving_licence[i]);
            }
        }else{
            formdata.append('driving_licence[]','');
        } 

        formdata.append('driving_licence_number',driving_licence_number); 
        formdata.append('link_request_from',link_request_from);

        // var driving_licence_doc = $("#driving_licence_doc").val(); 

        if (  driving_licence_number !='' &&  (driving_licence.length > 0 || driving_licence_doc !='' ) ) {
...
$("#add-driving-licence").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
            $.ajax({
                type: "POST",
                  url: base_url+"candidate/update_candidate_driving_licence",
                  data:formdata,
                  dataType: 'json',
                  contentType: false,
                  processData: false,
            ...
            if (driving_licence_number !=null) {
                valid_driving_licence_no();
            }
