        ...
        if (files[0].size <= client_document_size) {
	        ...
	        for (var i = 0; i < files.length; i++) {
	        	...
	        	if ((/\.(gif|jpg|jpeg|tiff|png|pdf)$/i).test(fileName)) {
	            	  html = '<div id="file_candidate_proof_'+i+'"><span>'+fileName+' <a id="file_candidate_voter'+i+'" onclick="removeFile_candidate_voter('+i+')" data-file="'+fileName+'" ><i class="fa fa-times text-danger"></i></a><a onclick="view_image_voter('+i+',\'candidate_voter\')" ><i class="fa fa-eye"></i></a></span></div>';
...
$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
		...
		if ( ( (candidate_aadhar.length != 0 && aadhar_number !='') || (passport_doc !='' && passport_doc !=null && passport_number !='' && passport_number !=null)) || ( (candidate_pan.length != 0 && pan_number !='') || (pan_card_doc !='' && pan_card_doc !=null  && pan_number !='')) || ((candidate_proof.length != 0   && passport_number !='') || (adhar_doc !='' && adhar_doc !=null  && aadhar_number !=''))  ) {
	$("#add-document-check").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
			...
			$.ajax({
	            type: "POST",
	              url: base_url+"candidate/update_candidate_document_check_insuff",
	              data:formdata,
	              dataType: 'json',
	              contentType: false,
	              processData: false,
		...
		if (candidate_aadhar.length == 0 && adhar_doc =='') { 
		...
		if (candidate_proof.length == 0 && passport_doc =='') { 
...
$("#aadhar_number").on('keyup blur',valid_aadhar_no);
$("#pan_number").on('keyup blur',valid_pan_no);
$("#passport_number").on('keyup blur',valid_passport_no);
