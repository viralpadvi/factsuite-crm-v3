...
// alert("appointment_letter.length")
 var designation = [];
...
// alert(appointment_letter.length)
// alert(experience_relieving_letter.length)
// alert(last_month_pay_slip.length)
...
// return false;
	...
	hr_name.length > 0 &&
	hr_contact.length > 0 && (candidate_aadhar.length != 0 ||(appointment_letter.length != 0 && appointment_letter !=null)) && (candidate_pan.length != 0 || (experience_relieving_letter.length != 0 && experience_relieving_letter !=null)) && (candidate_proof.length != 0  || (last_month_pay_slip.length != 0 && last_month_pay_slip !=null)) ) { 
...
$("#add_employments").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
	...
	$.ajax({
            type: "POST",
              url: base_url+"candidate/update_candidate_previous_employment_insuff",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
		...
		$(".designation").each(function(){
		   ...
		   var number = MyID.match(/\d+/); 
			...
			if (candidate_aadhar.length == 0 && appointment_letter=='') {
				$("#aadhar-error").html("<span class='text-danger error-msg-small'>Please upload appointment letter</span>");
			}
