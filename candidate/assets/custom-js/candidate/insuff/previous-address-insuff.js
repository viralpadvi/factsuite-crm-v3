...
duration_of_stay_start.length > 0 &&
duration_of_stay_end.length > 0 &&
contact_person_name.length > 0 &&
contact_person_mobile_number.length > 0  && (candidate_aadhar.length > 0 ||(rental_agreement.length > 0)) ) {
$("#add_address").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
	...
	/*	formdata.append('house',house);
		...
		formdata.append('ration_count',candidate_pan.length);
		formdata.append('gov_count',candidate_proof.length);

$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
	$.ajax({
            type: "POST",
              url: base_url+"candidate/update_candidate_previous_address_insuff",
              data:formdata,
              dataType: 'json',
              contentType: false,
              processData: false,
...
function valid_city(id){
	...
	if (city != '') {
		...
		$("#permenent-city-error"+id).html('<span class="text-danger error-msg-small">Please add a city.</span>');
		input_is_invalid("#permenent-city"+id);
	}	
}
