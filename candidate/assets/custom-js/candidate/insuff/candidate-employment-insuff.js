...
function valid_client_website() {
	...
	if (client_website != '') {
		if (!url_regex.test(client_website)) {
			$('#company_url-error').html('<span class="text-danger error-msg-small">Please enter the correct URL.</span>');
...
function input_is_invalid(input_id) {
 ...
 $("#add_employments").on('click',function(){
 	...
 	if (designation !='' &&
		...
		manager_contact !='' &&
		hr_name !='' &&
		hr_contact !='' && (candidate_aadhar.length != 0 ||(appointment_letter !='' && appointment_letter !=null)) && (candidate_pan.length != 0 || (experience_relieving_letter !='' && experience_relieving_letter !=null)) && (candidate_proof.length != 0  || (last_month_pay_slip !='' && last_month_pay_slip !=null)) ) { 
		 	// alert("Hello")
$("#add_employments").html('<div class="spinner-grow text-success spinner-grow-sm" role="status"><span class="sr-only">Loading...</span></div>Loading...');
		...
		$("#warning-msg").html("<span class='text-warning error-msg-small'>Please wait we are submitting the data.</span>");
			$.ajax({
		            type: "POST",
		              url: base_url+"candidate/update_candidate_employment_insuff",
		              data:formdata,
		              dataType: 'json',
		              contentType: false,
		              processData: false,
