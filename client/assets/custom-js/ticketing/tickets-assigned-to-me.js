...
function get_all_assigned_tickets() {
	$.ajax({
        type : "POST",
        url : base_url+"factsuite-client/get-all-assigned-tickets",
        data : {
        ...
        success: function(data) {
            ...
            if (data.all_tickets.length > 0) {
                for (var i = 0; i < all_tickets.length; i++) {
                    ...
                    html += '<td class="text-center" id="view_ticket_details_'+all_tickets[i].ticket_id+'"><a id="view_ticket_details_a'+all_tickets[i].ticket_id+'" href="javascript:void(0)" onclick="view_ticket_details('+all_tickets[i].ticket_id+')"><img src="'+img_base_url+'assets/client/assets-v2/dist/img/black-eye.svg"></a></td>';
                    html += '</tr>';
                }
            } else {
