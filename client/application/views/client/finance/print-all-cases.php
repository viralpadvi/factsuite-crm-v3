      ...
      <div id="FS-candidate-cnt" class="FS-candidate-cnt"> 
         ...
         <div class="table-responsive mt-3">
            ...
            foreach (explode(',', $case) as $key => $value) { 
               $data['candidate'] = $this->caseModel->getSingleAssignedCaseDetails($value);
               $data['candidate'] = $this->caseModel->clear_notification($summary_id);
               ...
               $analyst = array();
                ...
                foreach ($data['candidate'] as $k => $val) { 
                  ...
                  array_push($analyst,$val['candidate_id']);
                  if (!array_key_exists($val['candidate_id'], $candidate_data)) {
                     $candidate_data[$val['candidate_id']] = array( 
                        ...
                        $val['component_name'].' '.$val['formNumber']=>$price_value
                     );

                     $client = $val['client_name'];
                  }else{
                    $candidate_data[$val['candidate_id']][$val['component_name'].' '.$val['formNumber']] = $price_value; 
                  }
                    if (!array_key_exists($val['component_name'].' '.$val['formNumber'], $data_array)) {
                     $data_array[$val['component_name'].' '.$val['formNumber']] = array($price_value);
                    }else{
                     $data_array[$val['component_name'].' '.$val['formNumber']] = array_merge($data_array[$val['component_name'].' '.$val['formNumber']],array($price_value));
                    }
