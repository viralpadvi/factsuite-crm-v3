...
class AdminViewAllCaseModel extends CI_Model
{
	...
	function getAllAssignedCases() {
		...
		if($filter_input != '') {
			...
			$where_condition .= ' (`candidate_id` LIKE "'.$filter_input.'"';
		...
		$candidateData =  $this->db->join('tbl_client','candidate.client_id = tbl_client.client_id','left')->order_by('candidate_id','DESC')->limit($filter_limit)->get('candidate')->result_array();
		$data= array();
        ...
        // $form_values = json_decode($form_values2,true);
 		...
 		foreach ($component as $key => $value) {
 			...
 			foreach ($inputQcStatus as $inputQcStatuskey => $componentValue) {
				...
				/*if($result['tat_start_date'] != null && $result['tat_end_date'] != null && $result['tat_pause_date'] != null && $result['tat_pause_date'] != ''){
					$restart_date = 0;
					$start_date = 0;
					if($result['tat_re_start_date'] != null && $result['tat_re_start_date'] != ''){
						$restart_date = $this->number_of_working_days($result['tat_re_start_date'],date('d-m-Y'));
					}
					if($result['tat_pause_date'] !=null && $result['tat_pause_date'] !=''){
						$start_date = $this->number_of_working_days($result['tat_start_date'],$result['tat_pause_date']);
					}
					$total = $start_date + $restart_date;
					$row['left_tat_days'] = $total.' days';

				}else if($result['tat_start_date'] != null && $result['tat_end_date'] != null){
					if($result['tat_re_start_date'] != null && $result['tat_re_start_date'] != ''){
						$row['left_tat_days'] = $this->number_of_working_days($result['tat_re_start_date'],date('d-m-Y')).' days';
					}else{
						$row['left_tat_days'] = $this->number_of_working_days($result['tat_start_date'],date('d-m-Y')).' days';
					}
	 			...
	 			$row['case_generated_date'] = $result['report_generated_date']; 
