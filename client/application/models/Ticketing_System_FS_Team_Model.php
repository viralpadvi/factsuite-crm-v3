<?php
	...
	class Ticketing_System_FS_Team_Model extends CI_Model {
		...
		function get_logged_in_details($variable_array) {
			return $this->db->where('role_status','1')->where_in('role_id',5)->get('roles')->result_array();
		}
		...
		function get_roles_person_list($variable_array) {
			...
			if ($this->input->post('role_type') == 'csm') {
				$logged_in_client = $this->session->userdata('logged-in-client');
				$where_condition = array(
					'team_id' => $logged_in_client['account_manager_name']
				);
			} else {
		...
		function get_all_raised_tickets($variable_array) { 
			...
			$this->db->select('*');
			$this->db->from('raise_a_ticket');
			if ($variable_array['variable_array']['type'] == 'fs_team') {
				$where_condition = array(
					'ticket_created_by_role_id' => $variable_array['variable_array']['logged_in_user_details']['team_id'],
					'ticket_created_by_role' => $variable_array['variable_array']['logged_in_user_details']['role'],
				);
			}
			if ($variable_array['search']) {
				$keyword = $variable_array['search']['keyword'];
				if($keyword) {
					if (is_numeric($keyword)) {
						$this->db->where("ticket_id",$keyword);
					} else { 
						$this->db->where("ticket_subject LIKE '%$keyword%'");
					}
				}
			}
			$this->db->where($where_condition)->order_by('ticket_id','DESC');
			/*if($variable_array['count']){
			return $limit->num_rows();
			}else{
			$result = $limit->limit($variable_array['limit'], $variable_array['offset']);
			...
			if($variable_array['count']) {
				...
				return $this->db->limit($variable_array['limit'], $variable_array['offset'])->get()->result_array();
			}
		}

		function get_all_assigned_tickets($variable_array) {
			$where_condition = [];
			$this->db->select('*')->from('raise_a_ticket');
			if ($variable_array['variable_array']['type'] == 'fs_team') {
				$where_condition = array(
					'ticket_assigned_to_role_id' => $variable_array['variable_array']['logged_in_user_details']['team_id'],
					'ticket_assigned_to_role' => $variable_array['variable_array']['logged_in_user_details']['role'],
				);
			}
			if ($variable_array['search']) {
				$keyword = $variable_array['search']['keyword'];
				if($keyword) {
					if (is_numeric($keyword)) {
						...
						$this->db->or_where("ticket_subject LIKE '%$keyword%'");
						$this->db->or_where("ticket_description LIKE '%$keyword%'");
					}
				}
			}
			$this->db->where($where_condition)->order_by('ticket_id','DESC');
